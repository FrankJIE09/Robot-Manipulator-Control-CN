...
\tilde{f} = f - \hat{f} = f - \hat{\bm{W}}^T \sigma(\hat{\bm{V}}^T \bm{x})
\end{equation}
权重估计误差
\begin{equation}\label{eq:8.2.9}
\tilde{\bm{W}} = \bm{W} - \hat{\bm{W}}, \quad \tilde{\bm{V}} = \bm{V} - \hat{\bm{V}}
\end{equation}
和隐层输出误差
\begin{equation}\label{eq:8.2.10}
\tilde{\sigma} = \sigma - \hat{\sigma} = \sigma(\bm{V}^T \bm{x}) - \sigma(\hat{\bm{V}}^T \bm{x})
\end{equation}

对于任何$\bm{z}$，可以写出泰勒级数
\begin{equation}\label{eq:8.2.11}
\sigma(\bm{z}) = \sigma(\hat{\bm{z}}) + \sigma'(\hat{\bm{z}})\tilde{\bm{z}} + O(\tilde{\bm{z}})^2
\end{equation}
其中$\sigma'$是雅可比矩阵，最后一项表示$\tilde{\bm{z}}^2$阶的项。因此
\begin{equation}\label{eq:8.2.12}
\tilde{\sigma} = \sigma'(\hat{\bm{V}}^T \bm{x})\tilde{\bm{V}}^T \bm{x} + O(\tilde{\bm{V}}^T \bm{x})^2
\end{equation}
...
然后，使用调整规则~\eqref{eq:8.4.cond2}得
\begin{equation}\label{eq:8.4.tune6}
...
\bm{Z} \equiv \begin{bmatrix} \bm{W} & 0 \\ 0 & \bm{V} \end{bmatrix}, \quad \hat{\bm{Z}} \equiv \begin{bmatrix} \hat{\bm{W}} & 0 \\ 0 & \hat{\bm{V}} \end{bmatrix}, \quad \tilde{\bm{Z}} \equiv \bm{Z} - \hat{\bm{Z}}
\end{equation}
和$\bm{Z}$、$\hat{\bm{Z}}$等价。不难证明
\begin{equation}\label{eq:8.5.5}
\|\bm{w}\| \leq C_0 + C_1\|\tilde{\bm{Z}}\|_F + C_2\|\tilde{\bm{Z}}\|_F\|\bm{r}\|
\end{equation}
其中$C_i$是已知正常数。
...
\dot{\hat{\bm{V}}} = \bm{G}\bm{x}(\hat{\sigma}'^T\hat{\bm{W}}\bm{r})^T - \kappa\|\bm{r}\|\bm{G}\hat{\bm{V}}
\end{equation}
...
&\quad + \text{tr}\{\tilde{\bm{V}}^T(\bm{G}^{-1}\dot{\hat{\bm{V}}} + \bm{x}\bm{r}^T\hat{\bm{W}}^T\hat{\sigma}')\} \label{eq:8.5.subst8}
\end{align}
...
\dot{\hat{\bm{W}}} = \bm{F}\hat{\sigma}'\hat{\bm{V}}^T\bm{x}\bm{r}^T, \quad \dot{\hat{\bm{V}}} = \bm{G}\bm{x}(\hat{\sigma}'^T\hat{\bm{W}}\bm{r})^T
\end{equation}
...
相对于$\bm{W}$、$\bm{V}$的动力学为
\begin{equation}
\dot{\tilde{\bm{W}}} = -\bm{F}\hat{\sigma}'\hat{\bm{V}}^T\bm{x}\bm{r}^T, \quad \dot{\tilde{\bm{V}}} = -\bm{G}\bm{x}(\hat{\sigma}'^T\hat{\bm{W}}\bm{r})^T
\end{equation}
...
并沿轨迹(2)评估得
\begin{equation}
\dot{L} = \text{tr}\{\tilde{\bm{V}}^T \bm{G}^{-1} \dot{\tilde{\bm{V}}}\} = -\text{tr}\{\tilde{\bm{V}}^T \bm{x}\bm{r}^T\hat{\bm{W}}^T\hat{\sigma}'\} = \langle \bm{r}, \hat{\bm{W}}^T\hat{\sigma}'\hat{\bm{V}}^T\bm{x} \rangle
\end{equation}
...
\dot{\tilde{\bm{V}}} &= -\bm{G}\bm{x}\bm{r}^T\hat{\bm{W}}^T\hat{\sigma}' + \kappa\|\bm{r}\|\bm{G}\hat{\bm{V}}
\end{align}
