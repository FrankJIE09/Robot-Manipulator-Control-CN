...
\end{equation}
其中$q \in R^n$是机械臂关节变量；$\tau$为关节侧广义力矩。驱动连杆的电枢控制直流电机的动力学由$n$个解耦方程给出；$\tau_M$为电机轴侧负载力矩，与关节侧力矩满足$\tau_M = R\tau$（$R$为齿轮比矩阵，见(3.6.4)）。等价地可写成
\begin{equation}
...
使用(3.6.4)在(3.6.2)中消去$q_M$（有$\ddot{q}_M=R^{-1}\ddot{q}$，$\dot{q}_M=R^{-1}\dot{q}$），并将(3.6.1)中的$\tau = M\ddot{q}+V+F+G$代入(3.6.2)；两端左乘$R^{-1}$可整理为$\left(M + R^{-2}J_M\right)\ddot{q} + \left(V + R^{-2}B_M\right)\dot{q} + \left(F + R^{-1}F_M\right) + G = R^{-1}K_M v$。再将该式\textbf{两端左乘}$R^2$，并与记号$V_m \equiv V(q,\dot{q})$（见上文及表3.3.1中的科里奥利/向心项）、电机阻尼$B_M$并列书写，得到与图示等价的关节空间形式
\begin{equation}
\left(J_M + R^2 M\right)\ddot{q} + \left(B_M + R^2 V_m\right)\dot{q} + \left(R F_M + R^2 F\right) + R^2 G = R K_M v
\end{equation}
若(3.6.1)中尚含关节侧扰动$\tau_d$，则在上式左端加上$R^2\tau_d$，右端仍为$R K_M v$。或，通过适当定义符号，
\begin{equation}
M'(q)\ddot{q} + V'(q,\dot{q}) + F'(\dot{q}) + G'(q) = K' v
\end{equation}
其中$M'=J_M + R^2 M$，$V'=B_M + R^2 V_m$，$F'=R F_M + R^2 F(\dot{q})$，$G'=R^2 G(q)$，$K'=R K_M$；含扰动时左端加上$R^2\tau_d$。
...
\end{equation}
\begin{equation}
J_M\ddot{q}_M + B_M\dot{q}_M + F_M + R\tau = K_T I
\end{equation}
其中$I \in R^n$是电枢电流向量，
\begin{equation}
