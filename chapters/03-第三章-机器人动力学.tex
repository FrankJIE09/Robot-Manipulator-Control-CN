...
我们可以将连杆$i$的动能写为
\begin{equation}
K_i = \frac{1}{2}\sum_{j=1}^{i}\sum_{k=1}^{i}\text{trace}\left(\frac{\partial T_i}{\partial q_j}M_i\frac{\partial T_i^T}{\partial q_k}\right)\dot{q}_j\dot{q}_k
\end{equation}
...
回到我们的推导，机械臂总动能可以写为
\begin{equation}
K = \sum_{i=1}^{n}K_i = \frac{1}{2}\sum_{i=1}^{n}\sum_{j=1}^{n}\sum_{k=1}^{n}\text{trace}\left(\frac{\partial T_i}{\partial q_j}M_i\frac{\partial T_i^T}{\partial q_k}\right)\dot{q}_j\dot{q}_k
\end{equation}
...
用连杆$i$的伪惯量矩阵$M_i$与$e_4 = [0\ 0\ 0\ 1]^T$，也可写成$P = -\sum_{i=1}^{n} g^T T_i (M_i e_4)$，与上式在常规定义下等价。
...
对称矩阵$V_1(q)$、$V_2(q)$：
\begin{align}
V_1(q) &= \begin{bmatrix} -m_2a_1a_2\sin\theta_2 & -m_2a_1a_2\sin\theta_2 \\ -m_2a_1a_2\sin\theta_2 & 0 \end{bmatrix} \\
V_2(q) &= \begin{bmatrix} 0 & -m_2a_1a_2\sin\theta_2 \\ -m_2a_1a_2\sin\theta_2 & -m_2a_1a_2\sin\theta_2 \end{bmatrix}
\end{align}

位置/速度分解矩阵：
\begin{align}
V_p(q) &= \begin{bmatrix} -m_2a_1a_2\dot{\theta}_2\sin\theta_2 & -m_2a_1a_2(\dot{\theta}_1+\dot{\theta}_2)\sin\theta_2 \\ m_2a_1a_2\dot{\theta}_1\sin\theta_2 & 0 \end{bmatrix} \\
V_v(\dot{q}) &= \begin{bmatrix} 0 & -m_2a_1a_2\dot{\theta}_1\sin\theta_2 \\ -m_2a_1a_2(\dot{\theta}_1+\dot{\theta}_2)\sin\theta_2 & 0 \end{bmatrix}
\end{align}
...
\begin{equation}
\phi = \begin{bmatrix} m_1 \\ m_2 \\ m_1a_1 + m_2a_1 \\ m_2a_2 \\ m_1a_1^2 + m_2a_1^2 \\ m_2a_2^2 \\ m_2a_1a_2 \\ v_1 \\ v_2 \\ k_1 \\ k_2 \end{bmatrix}
\end{equation}

那么已知机器人函数的矩阵$W(q,\dot{q},\ddot{q})$变为
\begin{equation}
W = \begin{bmatrix} w_{11} & w_{12} & w_{13} & w_{14} & w_{15} & w_{16} & w_{17} & \dot{\theta}_1 & \text{sgn}(\dot{\theta}_1) & 0 & 0 \\ 0 & w_{22} & 0 & w_{24} & 0 & w_{26} & w_{27} & 0 & 0 & \dot{\theta}_2 & \text{sgn}(\dot{\theta}_2) \end{bmatrix}
\end{equation}
其中
\begin{align}
w_{11} &= \ddot{\theta}_1 \\
...
w_{13} &= g\cos\theta_1 \\
w_{14} &= g\cos(\theta_1 + \theta_2) \\
w_{15} &= a_1^2\ddot{\theta}_1 \\
w_{16} &= a_2^2(\ddot{\theta}_1 + \ddot{\theta}_2) \\
w_{17} &= (2\ddot{\theta}_1 + \ddot{\theta}_2)a_1a_2\cos\theta_2 - (2\dot{\theta}_1\dot{\theta}_2 + \dot{\theta}_2^2)a_1a_2\sin\theta_2 \\
w_{22} &= \ddot{\theta}_1 + \ddot{\theta}_2 \\
w_{24} &= g\cos(\theta_1 + \theta_2) \\
w_{26} &= a_2^2(\ddot{\theta}_1 + \ddot{\theta}_2) \\
w_{27} &= \ddot{\theta}_1 a_1a_2\cos\theta_2 + \dot{\theta}_1^2 a_1a_2\sin\theta_2
\end{align}
...
我们看到机械臂动力学可以表示为
\begin{equation}
\dot{x} = \begin{bmatrix} 0 & M^{-1}(q) \\ 0 & -V_m M^{-1} \end{bmatrix}x + \begin{bmatrix} 0 \\ I_n \end{bmatrix}u - \begin{bmatrix} 0 \\ G \end{bmatrix}
\end{equation}
其中$V_m$将$\dot{q}=M^{-1}p$代入原$V_m(q,\dot{q})$。控制输入定义为
\begin{equation}
u = \tau
\end{equation}
...
正如我们将在第4章看到的，反馈线性化提供了一种强大的控制设计技术。事实上，如果我们选择$u(t)$使得(3.4.35)稳定（例如，一种可能性是PD反馈$u = -K_v \dot{y} - K_p y$），那么由(3.4.37)定义的控制输入扭矩$\tau(t)$使机器人臂以$y(t)$变为零的方式移动。
...
从例3.2.3，机械臂惯量矩阵为
\begin{equation}
M = \begin{bmatrix} J + m_2r^2 & 0 & 0 \\ 0 & m_1 + m_2 & 0 \\ 0 & 0 & m_2 \end{bmatrix}
\end{equation}

应用(3.5.11)得到（验证！）
\begin{equation}
M' = \begin{bmatrix} \frac{J+m_2r^2}{r^2} & 0 & 0 \\ 0 & m_1+m_2 & 0 \\ 0 & 0 & m_2 \end{bmatrix}
\end{equation}
其中$J' = \frac{J}{r^2} + m_2$。
...
$n$-连杆机器人机械臂的方程来自表3.3.1（此处为与下文消元一致，先不显式写出干扰项$\tau_d$；需要时可在最终合成方程中统一加入）：
\begin{equation}
M(q)\ddot{q} + V(q,\dot{q}) + F(\dot{q}) + G(q) = \tau
\end{equation}
其中$q \in R^n$是机械臂关节变量；$\tau$为关节侧广义力矩。驱动连杆的电枢控制直流电机的动力学由$n$个解耦方程给出；$\tau_M$为电机轴侧负载力矩，与关节侧力矩满足$\tau_M = R\tau$（$R$为齿轮比矩阵，见(3.6.4)）。记$K'_m$为扭矩常数矩阵、$R_a$为电枢电阻矩阵（定义见(3.6.3)），电感可忽略时电枢电流的稳态近似给出如下等价写法：
\begin{equation}
J_M\ddot{q}_M + B_M\dot{q}_M + F_M + R\tau = K'_M v
\end{equation}
其中$\dot{q}_M = \text{vec}\{\dot{q}_{Mi}\}$，$\dot{q}_{Mi}$是第$i$个转子位置角，$\text{vec}\{\alpha_i\}$表示分量为$\alpha_i$的向量；$K'_M = R_a^{-1} K'_m$为电压到电机轴扭矩的有效对角增益矩阵。控制输入是电机电压向量$v \in R^n$。

执行器系数矩阵都是常数，由下式给出
\begin{equation}
\begin{aligned}
&J_M = \text{diag}\{J_{Mi}\}, \quad B_M = \text{diag}\{B_{Mi}\}, \quad K'_m = \text{diag}\{K_{Mi}\}, \quad R_a = \text{diag}\{R_{ai}\}, \\
&K'_M = R_a^{-1} K'_m = \text{diag}\{K_{Mi} R_{ai}^{-1}\}
\end{aligned}
\end{equation}
其中第$i$个电机具有惯量$J_{Mi}$、转子阻尼常数$B_{Mi}$、反电动势常数$K_{bi}$、扭矩常数$K_{Mi}$和电枢电阻$R_{ai}$。记号$K'_m$、$K'_M$在三阶模型(3.6.11)、(3.6.12)中沿用。
...
使用(3.6.4)在(3.6.2)中消去$q_M$（有$\ddot{q}_M=R^{-1}\ddot{q}$，$\dot{q}_M=R^{-1}\dot{q}$），并将(3.6.1)中的$\tau = M\ddot{q}+V+F+G$代入(3.6.2)；两端左乘$R^{-1}$可整理为$\left(M + R^{-2}J_M\right)\ddot{q} + \left(V + R^{-2}B_M\right)\dot{q} + \left(F + R^{-1}F_M\right) + G = R^{-1}K'_M v$。再将该式\textbf{两端左乘}$R^2$，并与记号$V_m \equiv V(q,\dot{q})$（见上文及表3.3.1中的科里奥利/向心项）、电机阻尼$B_M$并列书写，得到与图示等价的关节空间形式
\begin{equation}
\left(J_M + R^2 M\right)\ddot{q} + \left(B_M + R^2 V_m\right)\dot{q} + \left(R F_M + R^2 F\right) + R^2 G = R K'_M v
\end{equation}
若(3.6.1)中尚含关节侧扰动$\tau_d$，则在上式左端加上$R^2\tau_d$，右端仍为$R K'_M v$。或，通过适当定义符号，
\begin{equation}
M'(q)\ddot{q} + V'(q,\dot{q}) + F'(\dot{q}) + G'(q) = K' v
\end{equation}
其中$M'=J_M + R^2 M$，$V'=B_M + R^2 V_m$，$F'=R F_M + R^2 F(\dot{q})$，$G'=R^2 G(q)$，$K'=R K'_M$；含扰动时左端加上$R^2\tau_d$。

\textbf{完整机械臂加执行器动力学的性质}

完整动力学(3.6.6)与关节空间机器人动力学在结构上仍可对照表3.3.1逐项讨论。由于$M'=J_M+R^2M(q)$且$V'=B_M+R^2V_m$，其中$B_M$为常值对角阵，与纯连杆模型相比，验证斜对称/科里奥利项性质时须将$R^2V_m$与$B_M\dot{q}$分拆处理（见习题）。有界性及参数线性等结论在常见假设下仍适用。因此，在我们设计控制器时，往往仍以合并后的等效方程为对象，而把执行器视为已并入动力学中。
...
当电枢电感$L_i$不可忽略时，必须同时保留电流动力学，即用下列方程\textbf{代替}(3.6.2)所表示的电感略去模型（齿轮矩阵仍记为$R$，\textbf{电枢电阻}矩阵记为$R_a$以免混用）：
\begin{equation}
L\frac{dI}{dt} + R_a I + K_b'\dot{q}_M = v
\end{equation}
\begin{equation}
J_M\ddot{q}_M + B_M\dot{q}_M + F_M + R\tau = K'_m I
\end{equation}
其中$I \in R^n$为电枢电流向量；$K'_m=\mathrm{diag}\{K_{Mi}\}$与(3.6.3)中记号一致（电机电磁转矩为$K'_m I$；当$L_i$可略去时，由(3.6.10)稳态可得到(3.6.2)中$K'_M=R_a^{-1}K'_m$）。反电动势取$K_b'=\mathrm{diag}\{K_{bi}\}$。以下矩阵与(3.6.10)直接对应，并\textbf{约定}：与$\dot{q}_M$耦合的反电动势系数在归一化电流方程中统一写为$R_a^{-1}K_b'$，勿与左乘$K'_m$等改写后的系数混用。
\begin{equation}
\begin{aligned}
&L = \text{diag}\{L_i\}, \quad R_a = \text{diag}\{R_{ai}\}, \quad
T = L R_a^{-1} = \text{diag}\{L_i R_{ai}^{-1}\},\\
&K'_m = \text{diag}\{K_{Mi}\}, \quad
B_M = \text{diag}\{B_{Mi}\}, \quad
B' := R_a^{-1} K_b' = \text{diag}\{K_{bi} R_{ai}^{-1}\} 。\\
&\text{（将(3.6.10)两端左乘}R_a^{-1}\text{得归一化电流方程）}\quad
T\dot{I} + I + B'\dot{q}_M = R_a^{-1}v
\end{aligned}
\end{equation}
若采用SI单位且常取$K_{bi}=K_{Mi}$，则$B'=\mathrm{diag}\{K_{Mi}R_{ai}^{-1}\}$。其他文献有时把电气方程与$\tau=K'_m I$联立后再左乘常数矩阵，从而出现$\mathrm{diag}\{K_{bi}K_{Mi}R_{ai}^{-1}\}$等形式；与本书(3.6.10)逐项对照时，应以$R_a^{-1}K_b'$（即上式所定义之$B'$）为准。

重要的是要注意，$T$为电机电气时间常数矩阵。在前一小节中，假设这些时间常数与电机机械时间常数相比可以忽略不计。

为了确定机械臂加直流电机执行器的整体动力学，在(3.6.1)和(3.6.11)之间消去$\tau$以获得$I$的表达式。然后，求导以显式暴露$\dot{I}$。将这些表达式代入(3.6.10)（见习题）以获得形式为
\begin{equation}
...
\end{equation}
\begin{equation}
J_M\ddot{q}_M + B_M\dot{q}_M + F_M - B_s(\dot{q}_M - \dot{q}) - K_s(q_M - q) = K'_M v
\end{equation}
...
\begin{equation}
x = \begin{bmatrix} i \\ \theta_m \\ \omega_m \\ \theta_L \\ \omega_L \end{bmatrix}
\end{equation}
...
\begin{equation}
\dot{x} = \begin{bmatrix} -\frac{R}{L} & -\frac{K_b}{L} \\ \frac{K_T}{J} & -\frac{b_m}{J} \end{bmatrix}x + \begin{bmatrix} \frac{1}{L} \\ 0 \end{bmatrix}u
\end{equation}
...
传递函数计算为
\begin{equation}
H(s) = \frac{K_T}{(Ls+R)(Js+b_m) + K_T K_b}
\end{equation}
...
\item[3.6-2] \textbf{执行器动力学}。推导三阶动力学(3.6.13)，提供$\tau$、$N$的显式表达式。验证当$L_i$可忽略时它们简化为(3.6.5)。
