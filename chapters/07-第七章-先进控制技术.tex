...
M(q)\ddot{q} + N(q,\dot{q}) = K_T I
\end{equation}
\begin{equation}\label{eq:7.4.2}
L_a\dot{I} + R(q,\dot{q}) = u_E
\end{equation}
其中
...
\dot{\eta}_E = \bar{M}^{-1}(q)\dot{u}_L - \ddot{q}
\end{equation}

为了获得$\eta_E$的动态特性，我们从式\eqref{eq:7.4.1}代入$\ddot{q}$到式\eqref{eq:7.4.13}得到
\begin{equation}\label{eq:7.4.14}
\dot{\eta}_E = \bar{M}^{-1}(q)[\dot{u}_L - K_T\dot{I}]
\end{equation}

我们现在可以使用式\eqref{eq:7.4.14}设计输入$u_E$的控制律以迫使$\eta_E$趋近于零。$\eta_E$应该趋近于零的事实推动了校正控制律
\begin{equation}\label{eq:7.4.15}
u_E = L_a K_T^{-1}(\ddot{u}_L + K_{Ep}\eta_E) + R(\dot{q},I)
\end{equation}
...
\begin{tabular}{|l|l|}
\hline
\textbf{控制律} & $u_E = L_a K_T^{-1}(\ddot{u}_L + K_{Ep}\eta_E) + R(\dot{q},I)$ \\
\hline
\textbf{连杆控制器} & $u_L = \bar{M}(q)(\ddot{q}_d + K_{Lv}\dot{e} + K_{Lp}e) + N(q,\dot{q})$ \\
\hline
\textbf{扰动定义} & $\eta_E = \bar{M}^{-1}(q)(u_L - K_T I)$ \\
\hline
\end{tabular}
\end{center}

其中$\ddot{u}_L$通过微分$u_L$并从式\eqref{eq:7.4.1}代入$\ddot{q}$获得。
...
\begin{align}
u_E &= \frac{L_a}{K_T}(\ddot{u}_L + K_{Ep}\eta_E) + RI + k_b\dot{q}, \\
u_L &= \bar{m}(\ddot{q}_d + K_{Lv}\dot{e} + K_{Lp}e) + f_d \dot{q} + mgL \sin q
\end{align}
而$\ddot{u}_L$通过对$u_L$求导并从式(1)代入$\ddot{q}$得到
...
\begin{tabular}{|l|l|}
\hline
\textbf{控制律} & $u_F = J\bar{M}^{-1}(q)[\ddot{u}_L + K_{Fv}\dot{\eta}_F + \bar{M}(q)K_{Fp}\eta_F + \dot{\bar{M}}^{-1}(q)\bar{M}(q)\eta_F] + B_m(q_m,\dot{q}_m)$ \\
\hline
\textbf{连杆控制器} & $u_L = \bar{M}(q)(\ddot{q}_d + K_{Lv}\dot{e} + K_{Lp}e) + N(q,\dot{q})$ \\
\hline
\textbf{扰动定义} & $\eta_F = \bar{M}^{-1}(q)[u_L - K(q_m - q)]$ \\
\hline
\end{tabular}
\end{center}

其中$\dot{u}_L$和$\ddot{u}_L$通过微分$u_L$并从式\eqref{eq:7.4.28}代入$\ddot{q}$获得。
...
\begin{align}
u_F &= J\bar{m}^{-1}[\ddot{u}_L + K_{Fv}\dot{\eta}_F + \bar{m}K_{Fp}\eta_F] + B\dot{q}_m, \\
\eta_F &= \bar{m}^{-1}(u_L - K(q_m - q))
\end{align}
而$\dot{\eta}_F$可以通过对$\eta_F$求导得到。其中$\dot{u}_L$从式(1)中找到为
\begin{align}
