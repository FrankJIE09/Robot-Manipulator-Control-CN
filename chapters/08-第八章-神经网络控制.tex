...
\begin{aligned}
\tilde{f} &= f - \hat{f} = \bm{W}^T \sigma(\bm{V}^T \bm{x}) - \hat{\bm{W}}^T \sigma(\hat{\bm{V}}^T \bm{x}) + \varepsilon \\
&= \tilde{\bm{W}}^T \sigma(\hat{\bm{V}}^T \bm{x}) + \bm{W}^T [\sigma(\bm{V}^T \bm{x}) - \sigma(\hat{\bm{V}}^T \bm{x})] + \varepsilon \\
&= \tilde{\bm{W}}^T \hat{\sigma} + \bm{W}^T [\hat{\sigma}' \tilde{\bm{V}}^T \bm{x} + O(\tilde{\bm{V}}^T \bm{x})^2] + \varepsilon \\
&= \tilde{\bm{W}}^T [\hat{\sigma} - \hat{\sigma}' \hat{\bm{V}}^T \bm{x}] + \hat{\bm{W}}^T \hat{\sigma}' \tilde{\bm{V}}^T \bm{x} + \tilde{\bm{W}}^T \hat{\sigma}' \tilde{\bm{V}}^T \bm{x} \\
&\quad + \bm{W}^T O(\tilde{\bm{V}}^T \bm{x})^2 + \varepsilon
\end{aligned}
\end{equation}
...
\|\bm{x}\| \leq c_1 + c_2\|\bm{r}\| \leq q_B + c_0\|\bm{r}(0)\| + c_2\|\bm{r}\|
\end{equation}
...
S_r = \{\bm{r}(0): \|\bm{r}(0)\| < \frac{b_x - q_B}{c_0 + c_2}\}
\end{equation}
...
\end{equation}
其中$\bm{F}=\bm{F}^T>0$，$\kappa>0$是小设计参数。那么滤波跟踪误差$\bm{r}(t)$和神经网络权重估计$\hat{\bm{W}}(t)$是一致最终有界（UUB）的，实际界分别由式~上述不等式的右侧的右侧给出。而且，通过增加跟踪增益$K_v$，跟踪误差可以变得任意小。
...
定义Lyapunov函数候选并求导
\begin{align}
L &= \frac{1}{2}\bm{r}^T M(\bm{q})\bm{r} + \frac{1}{2}\text{tr}\{\tilde{\bm{W}}^T \bm{F}^{-1} \tilde{\bm{W}}\} \\
\dot{L} &= \bm{r}^T M \dot{\bm{r}} + \frac{1}{2}\bm{r}^T \dot{M} \bm{r} + \text{tr}\{\tilde{\bm{W}}^T \bm{F}^{-1} \dot{\tilde{\bm{W}}}\} \\
\intertext{代入式~\eqref{eq:8.4.2}得}
\dot{L} &= -\bm{r}^T (V_m + K_v)\bm{r} + \frac{1}{2}\bm{r}^T \dot{M} \bm{r} + \text{tr}\{\tilde{\bm{W}}^T(\bm{F}^{-1}\dot{\tilde{\bm{W}}} + \phi\bm{r}^T)\} + \bm{r}^T(\bm{\tau}_d + \varepsilon) \\
\intertext{然后，使用调整规则得}
\dot{L} &= -\bm{r}^T K_v \bm{r} + \kappa \|\bm{r}\| \text{tr}\{\tilde{\bm{W}}^T(\bm{W} - \tilde{\bm{W}})\} + \bm{r}^T(\bm{\tau}_d + \varepsilon)
\end{align}
...
因此，$\dot{L}$在紧集外是负的。根据增益选择条件确保由$\|\bm{r}\| \leq b_r$定义的紧集包含在$S_r$中，因此逼近性质在整个过程中成立。这证明了$\|\bm{r}\|$和$\|\bm{W}\|_F$的UUB。

这个证明类似于~\cite{narendra1987}，但修改为反映神经网络只在紧集上逼近的事实。在这个结果中，现在需要理想权重$\bm{W}$的界$W_B$，它出现在PD增益条件中。现在发现了跟踪误差$\bm{r}(t)$和权重$\bm{W}(t)$的界。注意，使用修改的权重调整算法建立$\bm{W}$的界不需要持续激励（PE）条件。

神经网络权重调整算法由反向传播时间项加上一个新的第二项组成。$\kappa$项（称为e-modification）的重要性在于它在$\|\tilde{\bm{W}}\|_F$中添加了二次项，使得可以证明$\dot{L}$在$(\|\bm{r}\|, \|\tilde{\bm{W}}\|_F)$平面上的紧集外为负~\cite{narendra1987}。e-mod项使调整律对未建模动力学具有鲁棒性，因此不需要PE条件。在~\cite{polycarpou1991}中，使用投影算法来保持神经网络权重有界。在~\cite{chen1992,chen1994}中，采用了死区技术。
...
\textbf{跟踪误差和神经网络权重估计误差的界}。跟踪误差不等式的右侧可以视为跟踪误差的实际界，即$\bm{r}(t)$永远不会偏离它太远。重要的是从这个方程注意到，跟踪误差随着神经网络重构误差$\varepsilon_N$和机器人扰动$d_B$而增加，但通过选择大增益$K_v$，可以获得任意小的跟踪误差。尽管Barron对$\varepsilon$有下界限制，也是如此。
...
M\dot{\bm{r}} = -(K_v + V_m)\bm{r} + \tilde{\bm{W}}^T\left(\hat{\sigma} - \hat{\sigma}'\hat{\bm{V}}^T\bm{x}\right) + \hat{\bm{W}}^T\hat{\sigma}'\tilde{\bm{V}}^T\bm{x} + \bm{w} + \bm{v}
\end{equation}
其中扰动项为
\begin{equation}\label{eq:8.5.3}
\bm{w} = \tilde{\bm{W}}^T\hat{\sigma}'\bm{V}^T\bm{x} + \bm{W}^T O(\tilde{\bm{V}}^T\bm{x})^2 + \varepsilon + \bm{\tau}_d
\end{equation}
...
\end{equation}
其中任意常数矩阵$\bm{F}=\bm{F}^T>0$，$\bm{G}=\bm{G}^T>0$，$\kappa>0$是小标量设计参数。那么滤波跟踪误差$\bm{r}(t)$和神经网络权重估计$\hat{\bm{V}}$、$\hat{\bm{W}}$是UUB的，界分别由式~\eqref{eq:8.5.8}和\eqref{eq:8.5.9}具体给出。而且，通过增加式~\eqref{eq:8.5.1}中的增益$K_v$，跟踪误差可以保持任意小。
...
定义Lyapunov函数候选并求导
\begin{align}
L &= \frac{1}{2}\bm{r}^T M(\bm{q})\bm{r} + \frac{1}{2}\text{tr}\{\tilde{\bm{W}}^T \bm{F}^{-1} \tilde{\bm{W}}\} + \frac{1}{2}\text{tr}\{\tilde{\bm{V}}^T \bm{G}^{-1} \tilde{\bm{V}}\} \\
\dot{L} &= \bm{r}^T M \dot{\bm{r}} + \frac{1}{2}\bm{r}^T \dot{M} \bm{r} + \text{tr}\{\tilde{\bm{W}}^T \bm{F}^{-1} \dot{\tilde{\bm{W}}}\} + \text{tr}\{\tilde{\bm{V}}^T \bm{G}^{-1} \dot{\tilde{\bm{V}}}\} \\
\intertext{从误差系统~\eqref{eq:8.5.2}代入得}
\begin{split}
\dot{L} &= -\bm{r}^T K_v \bm{r} + \frac{1}{2}\bm{r}^T (\dot{M} - 2V_m)\bm{r} - \bm{r}^T\bm{v} + \bm{r}^T\bm{w} \\
...
其中$K_{v\min}$是$K_v$的最小奇异值，最后一个不等式由于增益条件 $K_z > C_2$ 成立。
...
因此，$\dot{L}$在紧集外是负的。根据LaSalle扩展~\cite{lewis1993}，这证明了在该集合内控制保持有效时$\|\bm{r}\|$和$\|\bm{Z}\|_F$的UUB。然而，PD增益条件表明由$\|\bm{r}\| \leq b_r$定义的紧集包含在$S_r$中，因此逼近性质在整个过程中成立。

调整算法中的第一项不过是跟踪误差通过时间的反向传播。最后一项是Narendra e-mod，权重调整公式中的第二项是由于NLIP而产生的新型二阶项。鲁棒化项$\bm{v}(t)$也是由于NLIP问题而需要的。
...
\end{equation}
可以用于闭环控制代替上述权重调整公式。需要使用PE来证明使用直接反向传播时神经网络权重是有界的。提到的条件非常严格，在实践中不会发生；它们本质上要求机器人手臂是线性的（例如单连杆）。而且，在神经网络中不容易保证PE条件。
...
\bm{\xi}_1 = \tilde{\bm{W}}^T\left(\hat{\sigma} - \hat{\sigma}'\hat{\bm{V}}^T\bm{x}\right) + \hat{\bm{W}}^T\hat{\sigma}'\tilde{\bm{V}}^T\bm{x} + \bm{w}
\end{equation}
注意神经网络在误差动力学中的作用，其中它被分解为两个有效块，出现在典型的反馈配置中。

我们现在揭示直接反向传播调整~\eqref{eq:8.5.7}产生的无源性性质。为了证明这个算法，使用与~\eqref{eq:8.5.2}不同的误差动力学形式，使得在图8.5.1中$\bm{\xi}_1 = \tilde{\bm{W}}^T\sigma(\bm{V}^T\bm{x}) + \bm{w}$。
