...
其中
\[
M = \begin{bmatrix} m_1 & 0 \\ 0 & m_1 + m_2 \end{bmatrix}, \quad q = \begin{bmatrix} q_1 \\ q_2 \end{bmatrix}, \quad \tau = \begin{bmatrix} \tau_1 \\ \tau_2 \end{bmatrix}, \quad G = \begin{bmatrix} (m_1+m_2)g \\ 0 \end{bmatrix}
\]
...
\dot{V} = \dot{q}^T M(q)\ddot{q} + \frac{1}{2}\dot{q}^T \dot{M}(q)\dot{q} - \dot{x}^T K_p \tilde{x} + \dot{x}^T K_e(x - x_e)
\end{equation}
注意，在式(\ref{eq:9.2.21})中，我们使用了$x_e$和$x_d$是常数的事实，并且标量函数或对角矩阵的转置等于该函数或矩阵。将式(\ref{eq:9.2.19})代入式(\ref{eq:9.2.21})并利用式(\ref{eq:9.2.10})以及斜对称性质（见第2章）得到
...
J^T(q)K_e(x - x_e) = J^T(q)K_p \tilde{x}
\end{equation}
或等价地
...
上面的稳定性分析可以解释为意味着当任务空间坐标由式(\ref{eq:9.2.24})给出时，机器人机械臂将停止移动。也就是说，末端执行器的最终位置或稳态位置由式(\ref{eq:9.2.24})给出，这在单自由度情况下等价于由式(\ref{eq:9.2.6})给出。为了获得施加在环境上的稳态力的第$i$个分量，我们将式(\ref{eq:9.2.24})代入式(\ref{eq:9.2.17})的第$i$个分量，得到
\begin{equation}\label{eq:9.2.25}
f_{iss} = \frac{K_{pi}K_{ei}}{K_{pi} + K_{ei}}(x_{di} - x_{ei})
\end{equation}

因此末端执行器施加在环境上的稳态力由式(\ref{eq:9.2.25})给出，这在单自由度情况下等价于由式(\ref{eq:9.2.7})给出。如在单自由度情况下一样，我们假设对于要力控制的任务空间方向，$K_{ei}$远大于$K_{pi}$。也就是说，式(\ref{eq:9.2.25})中的稳态力可以近似为
\begin{equation}\label{eq:9.2.26}
f_{iss} \approx K_{pi}(x_{di} - x_{ei})
\end{equation}
因此，$K_{pi}$可以被解释为在这些任务空间方向上指定机械臂的刚度。

如果机械臂在任务空间方向上不受约束，则相应的刚度常数$K_{ei}$等于零。将$K_{ei} = 0$代入式(\ref{eq:9.2.24})得到
...
\begin{align}
f_{d1} &= k_p(u_d - u_e) \\
2 &= 10\left(u_d - \frac{1}{\sqrt{2}}\right) \\
u_d &= 0.3414
\end{align}
...
\tau_2 = (m_1 + m_2)(a_M + k_{Tv}\dot{\tilde{x}} + k_{Tp}\tilde{x}) + (m_1 + m_2)g + f_2
\end{equation}
其中
...
\tau_1 = m_1g + \frac{m_1}{k_e}a_N + f_1
\end{equation}
其中
...
a_{Nj} = \frac{1}{k_{ej}}(\ddot{f}_{Ndj} + k_{Nvj}\dot{\tilde{f}}_{Nj} + k_{Npj}\tilde{f}_{Nj})
\end{equation}
...
\tau = M\Sigma(\ddot{x}_d + K_v\dot{\tilde{x}} + K_p \tilde{x}) + N + A^T(\lambda_d + K_f \tilde{\lambda})
\end{equation}
...
M\Sigma(\ddot{\tilde{x}} + K_v\dot{\tilde{x}} + K_p \tilde{x}) + A^T(\tilde{\lambda} + K_f \tilde{\lambda}) = 0
\end{equation}
