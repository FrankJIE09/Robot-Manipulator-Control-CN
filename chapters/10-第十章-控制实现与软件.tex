...
\begin{align}
\underline{w}\underline{x} = 0; \quad \underline{w} &= \left[\begin{array}{c} \underline{n} \\ d_0 \end{array}\right] \label{eq:10.5.1a} \\
d = \underline{w}^T\underline{x}_{EndEffector} &= \left\{\begin{array}{ll} <0 & \text{末端执行器在墙壁外} \\ >0 & \text{末端执行器在墙壁内} \end{array}\right. \label{eq:10.5.1b} \\
\tau &= \left\{\begin{array}{cc} 0 & d<0 \text{（末端执行器在墙壁外）} \\ J^T(k_f d\underline{n}) & d>0 \text{（末端执行器在墙壁内）} \end{array}\right. \label{eq:10.5.1c}
\end{align}

其中，$\underline{n}$是平面的法向量，$d_0$是距原点的距离；$\underline{x}_{EndEffector}$是当前末端执行器位置；式\eqref{eq:10.5.1c}为控制律。
