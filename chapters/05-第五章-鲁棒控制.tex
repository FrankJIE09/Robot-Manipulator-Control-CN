...
M(q)\ddot{q} + V_m(q, \dot{q})\dot{q} + F(\dot{q}) + G(q) = \tau - \tau_d
\end{equation}
...
p = q + w_i, \quad \dot{p} = \dot{q} + w_{i+1}
\end{equation}
...
为简化起见，我们假设式(\ref{eq:5.1.1})中 $\tau_d = 0$，式(\ref{eq:5.1.3})中 $w_i = 0$，尽管可以轻松考虑有界的 $\tau_d$ 和 $w_i$ 的影响，并在大多数示例中加以考虑。类似于第4章，机器人的动力学被转换为线性系统
\begin{equation*}
\begin{bmatrix} \dot{e} \\ \ddot{e} \end{bmatrix} = \begin{bmatrix} 0 & I \\ 0 & 0 \end{bmatrix} \begin{bmatrix} e \\ \dot{e} \end{bmatrix} + \begin{bmatrix} 0 \\ I \end{bmatrix} u
\end{equation*}

和

\begin{equation}\label{eq:5.2.1}
u = M(q)^{-1}[N(q, \dot{q}) - \tau] + \ddot{q}_d
\end{equation}

导出非线性计算力矩控制器

\begin{equation}\label{eq:5.2.2}
\tau_d = M(q)[\ddot{q}_d - u] + N(q, \dot{q})
\end{equation}
...
E(s) = \frac{1}{s^2}I\,U(s)
\end{equation}

然后问题简化为找到线性控制 $u$ 来实现期望的闭环性能；即，求出 $F$、$G$、$H$ 和 $J$，使
\begin{align}
\dot{z} &= Fz + Ge \\
u &= Hz + Je
\end{align}
...
u(t) = [H(sI - F)^{-1}G + J]e(t) \equiv C(s)e(t).
\end{equation}

注意上述概念表明 $u(t)$ 是当输入 $e(t)$ 施加时系统 $C(s)$ 的输出。还请注意，从式(\ref{eq:5.2.4})可以看出，机器人的不同关节是去耦的，因此在这一层次上，可以设计 $n$ 个独立的SISO控制器来控制机器人的 $n$ 个关节。

不幸的是，控制律(\ref{eq:5.2.2})通常由于复杂性或 $M(q)$ 和 $N(q, \dot{q})$ 中存在的不确定性以及 $\tau_d$ 和 $w_i$ 的存在而无法实现。相反，应用式(\ref{eq:5.2.6})中的 $\tau$，其中 $\hat{M}$ 和 $\hat{N}$ 是 $M$ 和 $N$ 的估计值：

\begin{equation}\label{eq:5.2.6}
\tau = \hat{M}[\ddot{q}_d - u] + \hat{N}
\end{equation}
...
\begin{aligned}
\dot{e} &= Ae + B(u + \eta) \\
\eta &= \Delta(u - \ddot{q}_d) + M^{-1}\delta \\
\Delta &= M^{-1}\hat{M} - I_n, \quad \delta = N - \hat{N}.
\end{aligned}
\end{equation}

在 $\hat{M} = M$ 且 $\hat{N} = N$ 时，有 $\Delta = 0$、$\delta = 0$，因此 $\eta = 0$。然而，一般而言，向量 $\eta$ 是 $e$ 和 $u$ 的非线性函数，不能视为外部干扰。它代表由建模不确定性、参数变化、外部干扰、摩擦项甚至可能是测量噪声引起的全局线性化误差动力学的内部干扰[Spong and Vidyasagar 1987]。
...
\frac{1}{\mu_2} \leq \|M^{-1}\| \leq \frac{1}{\mu_1}, \quad \mu_1 \neq 0
\end{equation}

\begin{equation}\label{eq:5.2.9}
\|\Delta\| \leq \alpha \leq 1
\end{equation}

在使用这种方法时，通常对旋转关节机器人做出合理的假设(\ref{eq:5.2.9})--(\ref{eq:5.2.11})[Spong and Vidyasagar 1987]。在下文中，$\mu_1$、$\mu_2$、$\alpha$、$\beta_0$、$\beta_1$ 和 $\beta_2$ 是依赖于不确定性大小的非负有限常数。

\begin{equation}\label{eq:5.2.10}
\|\delta\| \leq \beta_0 + \beta_1\|e\| + \beta_2\|e\|^2
\end{equation}
...
回顾不等式(\ref{eq:5.2.8})在第3.3节中引入，并注意上述不等式中使用的范数可以是 $\infty$ 范数或 2 范数，取决于应用。还注意，对于具有棱柱关节的机器人，界限 $\mu_i$ 与 $\alpha_i$ 为 $q$ 的标量函数，并且通过选择 $\hat{M} = \tfrac{1}{2}(\mu_1 + \mu_2)I$，可使式(\ref{eq:5.2.9})成立且 $\alpha = (\mu_2 - \mu_1)/(\mu_2 + \mu_1)$ [Spong and Vidyasagar 1987]。最后，(\ref{eq:5.2.10})是第3.3节讨论的科里奥利和向心项性质的结果。
...
\dot{e} = Ae + B(u + \eta) = (A - BK)e + B\eta = A_c e + B\eta
\end{equation}

可以看出，通过将 $A_c$ 的极点足够远地放置在左半平面，可以保证在存在 $\eta$ 的情况下闭环系统的鲁棒稳定性。这在[Arimoto and Miyazaki 1985]中对于 $\dot{q}_d = 0$、$\tau_d = 0$ 的情况被证明是正确的，如定理4.4.1和例4.4.3中所述。对于轨迹跟踪问题，在[Dawson et al. 1990]中在假设 $e(0) = \dot{e}(0) = 0$ 下也证明了这一点，如定理4.4.2中所述。
...
\begin{align}
\dot{e} &= \begin{bmatrix} 0 & I \\ -K_p & -K_v \end{bmatrix} e + \begin{bmatrix} 0 \\ I \end{bmatrix} u \label{eq:5.2.14} \\
y &= \begin{bmatrix} K_p & K_v \end{bmatrix} \begin{bmatrix} e \\ \dot{e} \end{bmatrix} = Ke
\end{align}
...
Q = \begin{bmatrix} 2K_p^2 & 0 \\ 0 & 2K_v^2 - 2K_p \end{bmatrix}
\end{equation}

使得

\begin{equation}\label{eq:5.2.17}
P = \begin{bmatrix} 2K_p K_v & K_p \\ K_p & K_v \end{bmatrix}
\end{equation}
...
K = B^T P = \begin{bmatrix} K_p^2/a & K_v \end{bmatrix}, \quad a > 1
\end{equation}
...
\begin{equation}
a > 1 + \frac{1}{\mu_1}[\beta_1 + 2(\beta_2\beta_0 + \beta_2(\mu_1 + \mu_2)c)^{\frac{1}{2}}]
\end{equation}

其中 $K_v = 2aI$ 和 $K_p = 4aI$。
\end{theorem}

\begin{proof}
考虑式(\ref{eq:5.2.13})给出的闭环系统与控制器(\ref{eq:5.2.12})，并选择以下李雅普诺夫函数候选：

\begin{equation}
V = \frac{1}{2}e^T P e + \int_0^t y^T(\sigma)\Delta y(\sigma) d\sigma
\end{equation}

其中 $\frac{1}{2}e^T P e$ 是对应于SPR系统(\ref{eq:5.2.14})的李雅普诺夫函数。那么如果 $\Delta \geq 0$，我们有 $V > 0$。当 $\hat{M} \geq \mu_2 I$ 时满足此条件。对其求导可得

\begin{equation}
\dot{V} = -\frac{1}{2}e^T P e + e^T K(M^{-1}\delta - \Delta\ddot{q}_d)
\end{equation}

为了保证 $\dot{V} < 0$，回忆边界(\ref{eq:5.2.9})--(\ref{eq:5.2.11})，并写出

\begin{equation}
\dot{V} \leq -\frac{q_1}{2}\|e\|^2 + \frac{k}{\mu_1}[\beta_2\|e\|^3 + \beta_1\|e\|^2 + (\beta_0 + (\mu_2 + \mu_1)c)\|e\|]
\end{equation}

其中 $q_1 = \lambda_{\min}(Q) = 2(a-1)k$，$(a-1)I < K_p = kI$，$K_v = 2aI < K_p = 4aI < K_v^2$。注意可以从(3)中提出因子 $\|e\|$而不影响方程的符号确定性。误差的均匀有界性然后使用引理2.10.3和定理2.10.3得到保证，如果

\begin{equation}
k\beta_2\|e\|^2 + \left(k\beta_1 - \mu_1\frac{q_1}{2}\right)\|e\| + k[\beta_0 + (\mu_1 + \mu_2)c] < 0
\end{equation}

这可以通过以下条件保证：

\begin{equation}
q_1 > 2\frac{k}{\mu_1}[\beta_1 + 2(\beta_2\beta_0 + \beta_2(\mu_1 + \mu_2)c)^{1/2}]
\end{equation}

或

\begin{equation}
a > 1 + \frac{1}{\mu_1}[\beta_1 + 2(\beta_2\beta_0 + \beta_2(\mu_1 + \mu_2)c)^{1/2}]
\end{equation}

误差将被一个随着 $a$ 增大而趋于零的项所限制（有关详细信息，请参见定理2.10.3及其在[Dawson et al. 1990]中的证明）。这一分析允许 $\Delta$ 任意大，只要 $\hat{M} \geq \mu_2 I$，如下例所示。事实上，如果 $N$ 是已知的，由于在这种情况下 $\delta = 0$，从被动性定理可以确保全局渐近稳定性。控制器总结在表5.2.1中。

研究(6)并尝试理解其各项的贡献是有启发性的。以下选择将有助于满足(6)：
\begin{enumerate}
    \item 对应于大 $a$ 的大增益 $K_p$ 和 $K_v$。
    \item 对 $N$ 的良好了解，这转化为小的 $\beta_i$。
    \item 大的 $\mu_1$ 或大的惯性矩阵 $M(q)$。
    \item 具有小 $c$ 的轨迹，即小的期望加速度 $\ddot{q}_d$。
\end{enumerate}
\end{proof}

以下示例说明了定理\ref{thm:5.2.1}中关于增益的充分性条件以及更大增益 $K_p$ 和 $K_v$ 的影响。
...
\begin{equation*}
\mu_1 = 0.1, \quad \mu_2 = 6
\end{equation*}

让 $\hat{N} = 0$ 然后

\begin{equation*}
\|\delta\| \leq \sqrt{10}\|e\|^2 + 2\sqrt{10}\|e\| + \sqrt{10}
\end{equation*}

或

\begin{equation*}
\beta_0 = \sqrt{10}, \quad \beta_1 = 2\sqrt{10}, \quad \beta_2 = \sqrt{10}
\end{equation*}

然后使用 $\hat{M} = 6I$ 和 $a = 172$ 来满足定理\ref{thm:5.2.1}。事实上，这些值将导致比实际需要的更大的控制器增益。相反，假设我们让 $\hat{M} = 6I$，$\hat{N} = 0$，并且
...
\textbf{表5.2.1}\quad 静态控制器，李雅普诺夫设计

\begin{center}
\begin{tabular}{|l|l|}
\hline
\multicolumn{2}{|l|}{\textbf{假设：}} \\
\hline
$\dfrac{1}{\mu_2} \leq \|M^{-1}\| \leq \dfrac{1}{\mu_1}, \quad \mu_i \neq 0$ & \\
$\|\Delta\| \leq a \leq 1$ & \\
$\|\delta\| \leq \beta_0 + \beta_1\|e\| + \beta_2\|e\|^2$ & \\
$\|\ddot{q}_d\| \leq c$ & \\
$e(0) = \dot{e}(0) = 0$ & \\
\hline
\multicolumn{2}{|l|}{\textbf{控制器：}} \\
\hline
$\tau = \hat{M}(\ddot{q}_d + 2a\dot{e} + 4ae) + \hat{N}$ & \\
$\hat{M} > \mu_2 I$ & \\
$a > 1 + \dfrac{1}{\mu_1}\left[\beta_1 + 2(\beta_2\beta_0 + \beta_2(\mu_1 + \mu_2)c)^{\frac{1}{2}}\right]$ & \\
\hline
\multicolumn{2}{|l|}{\textbf{性能：}} \\
\hline
\multicolumn{2}{|p{10cm}|}{保证$e$和$\dot{e}$的李雅普诺夫稳定性。当$a$增大时，误差趋于零。} \\
\hline
\end{tabular}
\end{center}
...
这些控制器具有与第4.4节和前一节中描述的相同的结构。不同之处在于这里我们使用输入-输出概念而不是李雅普诺夫理论暗示的状态空间方法来显示误差的稳定性。在[Craig 1988]中，使用静态控制器展示了误差信号的有界性。(\ref{eq:5.2.9})--(\ref{eq:5.2.10})中使用的范数然后是 $L_\infty$ 范数。该控制器的开发始于假设(\ref{eq:5.2.9})和(\ref{eq:5.2.10})到

\begin{equation}\label{eq:5.2.20}
\|\delta\| \leq \beta_0 + \beta_1\|e\| + \beta_2\|\dot{e}\|^2
\end{equation}
...
\begin{bmatrix} E(s) \\ sE(s) \end{bmatrix} = \begin{bmatrix} (s^2 + sK_v + K_p)^{-1} \\ s(s^2 + sK_v + K_p)^{-1} \end{bmatrix} \eta(s) = \begin{bmatrix} P_{11}(s) \\ P_{12}(s) \end{bmatrix}
\end{equation}
...
可以看出，$K_v$ 和 $K_p$ 都是对角矩阵，$K_v^2 = 4K_p$，在每个关节实现临界阻尼响应[参见(4.4.22)、(4.4.30)和(3.3.32)]。$P_{11}(s)$ 和 $P_{12}(s)$ 的无限算子增益是（参见引理2.5.2和例2.5.8）：

\begin{equation}\label{eq:5.2.25}
\gamma_\infty(P_{11}) \equiv \gamma_{11} = \frac{1}{k_p}, \quad \gamma_\infty(P_{12}) \equiv \gamma_{12} = \frac{4}{ek_v}
\end{equation}

其中

\begin{equation}\label{eq:5.2.26}
k_p = \max\{K_{pi}\}, \quad k_v = \max\{K_{vi}\}, \quad e = 2.71828
\end{equation}

然后考虑以下不等式：

\begin{align}
\|e\|_{T\infty} &\leq \gamma_{11}\|\eta\|_{T\infty} \\
\|\dot{e}\|_{T\infty} &\leq \gamma_{12}\|\eta\|_{T\infty}
\end{align}

使用(\ref{eq:5.2.9})--(\ref{eq:5.2.11})，我们有

\begin{equation}\label{eq:5.2.27}
\|\eta\|_{T\infty} \leq ac + ak_p\|e\|_{T\infty} + ak_v\|\dot{e}\|_{T\infty} + \frac{\beta_0}{\mu_1} + \frac{\beta_1}{\mu_1}\|\dot{e}\|_{T\infty} + \frac{\beta_2}{\mu_1}\|\dot{e}\|_{T\infty}^2
\end{equation}
...
\begin{equation}
\gamma_{12}\left(ak_v + \frac{\beta_1}{\mu_1}\right) = \frac{4a}{e} + \frac{4\beta_1}{e\mu_1 k_v} < 1
\end{equation}

那么误差的 $L_\infty$ 稳定性就得到保证，如果

\begin{equation}
1 - \gamma_{11}ak_p - \gamma_{12}\left(ak_v + \frac{\beta_1}{\mu_1}\right) - 2\gamma_{12}\left[\left(a\|\ddot{q}_d\| + \frac{\beta_0}{\mu_1}\right)\frac{\beta_2}{\mu_1}\right]^{\frac{1}{2}} > 0
\end{equation}
\end{theorem}
...
\begin{equation}
1 - a\left(1 + \frac{4}{e}\right) - \frac{4\beta_1}{\mu_1 e k_v} - 2\frac{4}{e k_v}\left[\left(a\|\ddot{q}_d\| + \frac{\beta_0}{\mu_1}\right)\frac{\beta_2}{\mu_1}\right]^{\frac{1}{2}} > 0
\end{equation}

并进一步简化为

\begin{equation}\label{eq:5.2.28}
k_v = \frac{4\beta_1 - 8[(a\mu_1 c + \beta_0)\beta_2]^{\frac{1}{2}}}{\mu_1 e[1 - a(1 + \frac{4}{e})]}
\end{equation}

让我们研究上述不等式以确定各项的影响。以下观察有助于满足(\ref{eq:5.2.28})。
\begin{enumerate}
    \item 大的 $k_v$ 有助于满足稳定性条件。注意：这也意味着大的 $k_p$。
    \item 对 $N$ 的良好了解，这将转化为小的 $\beta_i$。
    \item 大的 $\mu_1$ 或大的惯性矩阵 $M(q)$。
...
\begin{table}[htbp]
\centering
\caption{静态控制器，输入-输出设计}
\label{tab:5.2.2}
\begin{tabular}{@{}l@{}}
\toprule
\textbf{假设:} \\
\quad $\displaystyle \frac{1}{\mu_2} \leq \|M^{-1}\| \leq \frac{1}{\mu_1}, \quad \mu_i \neq 0$ \\[1em]
\quad $\|\Delta\| \leq a \leq 1$ \\[0.8em]
\quad $\|\delta\| \leq \beta_0 + \beta_1\|e\| + \beta_2\|e\|^2$ \\[0.8em]
\quad $\|\ddot{q}_d\| \leq c$ \\[0.8em]
\quad $e(0) = \dot{e}(0) = 0$ \\[1em]
\midrule
\textbf{控制器:} \\
\quad $\displaystyle \tau = \hat{M}\left(\ddot{q}_d + k_v\dot{e} + \frac{k_v^2}{4}e\right) + \hat{N}$ \\[1.2em]
\quad $\displaystyle 1 - a\left(1 + \frac{4}{e}\right) - \frac{4\beta_1}{\mu_1 e k_v} - 2\frac{4}{e k_v}\left[\left(a\|\ddot{q}_d\| + \frac{\beta_0}{\mu_1}\right)\frac{\beta_2}{\mu_1}\right]^{\frac{1}{2}} > 0$ \hfill (5.2.29) \\[1em]
\midrule
\textbf{性能:} \\
\quad $e$ 和 $\dot{e}$ 的有界性得到保证。 \\
\quad 随着 $k_v$ 的增加，误差趋于零。 \\
\bottomrule
\end{tabular}
\end{table}
...
条件(1)在 $k_v > 720$ 时满足。这当然是一个大界限，可以通过选择更好的 $\hat{N}$ 来改进。对于 $k_p = 225$ 和 $k_v = 30$ 的闭环行为仿真如图5.2.5所示。误差幅值比使用可比较控制努力的例5.2.1的PD控制器所实现的误差要小得多。这种改进是以了解式(1)中的惯性矩阵 $M(q)$ 为代价的。
\end{example}
...
e = G(I - CG)^{-1}\eta \equiv P_1\eta
\end{equation}

并定义

\begin{equation}\label{eq:5.2.31}
u = CG(I - CG)^{-1}\eta \equiv P_2\eta
\end{equation}

令 $P_i$、$i = 1, 2$ 的算子增益由下式给出

\begin{equation}\label{eq:5.2.32}
\gamma_\infty(P_i) = \gamma_i, \quad i = 1, 2
\end{equation}

注意 $\gamma_1 = \max(\gamma_{11}, \gamma_{12})$。参见引理2.5.2和例2.5.8。然后考虑

\begin{align}
\|e\|_{T\infty} &\leq \gamma_1\|\eta\|_{T\infty} \\
\|u\|_{T\infty} &\leq \gamma_2\|\eta\|_{T\infty} \\
\|\eta\|_{T\infty} &\leq a\|\ddot{q}_d\|_{T\infty} + a\|u\|_{T\infty} + \frac{\beta_0}{\mu_1} + \frac{\beta_1}{\mu_1}\|e\|_{T\infty} \label{eq:5.2.33}
\end{align}

下一个定理给出了轨迹误差BIBO稳定性的充分条件。

\begin{theorem}\label{thm:5.2.3}
定理 5.2--3：闭环系统(5.2.30)的BIBO稳定性将得到保证，如果

\begin{equation}
1 - \frac{\beta_1\gamma_1}{\mu_1} - a\gamma_2 > 0
\end{equation}

事实上，轨迹误差受以下限制：

\begin{align}
\|e\|_\infty &\leq \frac{\gamma_1 b}{1 - \beta_1\gamma_1/\mu_1 - a\gamma_2} \\
b &= a\|\ddot{q}_d\|_{T\infty} + \frac{\beta_0}{\mu_1}
\end{align}
\end{theorem}
...
\midrule
\textbf{控制器:} \\
\quad $\tau = \hat{M}(\ddot{q}_d + C(s)e) + \hat{N}$ \\[1em]
\quad \textbf{with} \\[0.5em]
\quad $\|e\|_{T\infty} \leq \gamma_1\|\eta\|_{T\infty}$ \\[0.5em]
\quad $\|u\|_{T\infty} \leq \gamma_2\|\eta\|_{T\infty}$ \\[0.5em]
\quad $\|\eta\|_{T\infty} \leq a\|\ddot{q}_d\|_{T\infty} + a\|u\|_{T\infty} + \frac{\beta_0}{\mu_1} + \frac{\beta_1}{\mu_1}\|e\|_{T\infty}$ \\[0.8em]
\quad $1 - \frac{\beta_1\gamma_1}{\mu_1} - a\gamma_2 > 0$ \\[1em]
\midrule
\textbf{性能:} \\
\quad $\displaystyle \|e\|_\infty \leq \frac{\gamma_1 b}{1 - \beta_1\gamma_1/\mu_1 - a\gamma_2}$ \\[0.8em]
\quad $\displaystyle b = a\|\ddot{q}_d\|_{T\infty} + \frac{\beta_0}{\mu_1}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{equation*}
N(s) = \tilde{N}(s) = \frac{1}{(s+1)^2}\begin{bmatrix} I \\ sI \end{bmatrix}
\end{equation*}

\begin{equation*}
D(s) = \frac{s^2}{(s+1)^2}I = \tilde{D}(s)
\end{equation*}

\begin{equation*}
\tilde{D}(s) = \frac{1}{(s+1)^2}\begin{bmatrix} (s^2+2s)I & -2I \\ -sI & (s^2+1)I \end{bmatrix}
\end{equation*}
...
\begin{equation*}
X(s) = \frac{1}{(s+1)^2}\begin{bmatrix} (1+2s)I & (2+4s)I \end{bmatrix}, \quad Y(s) = \frac{s^2+4s+2}{(s+1)^2}I
\end{equation*}
...
\begin{equation*}
C(s) = \frac{2s+1}{s^2+4s+2}\begin{bmatrix} I & 2I \end{bmatrix}
\end{equation*}

还可以选择 $Q(s)$ 来满足所需的性能。特别是，[Spong and Vidyasagar 1987]中提出了以下 $Q(s)$ 的选择：

\begin{equation*}
Q(s) = \begin{bmatrix} 2I & \dfrac{4k+(k+2)s}{s+k}I \end{bmatrix}, \quad k=1,2,3,\ldots
\end{equation*}
...
\begin{align*}
C_1(s) &= -\frac{2s^3+(k+4)s^2+(2k+1)s+k}{s^2(s+2)}I \\
C_2(s) &= -\frac{(k+2)s^3+4(k+1)s^2+5ks+2k}{s^2(s+2)}I
\end{align*}

和

\begin{equation*}
u = C_1(s)e + C_2(s)\dot{e}
\end{equation*}
...
定理 5.2--4：式(5.2.30)的误差系统是 $\mathcal{L}_\infty$ 有界的，如果
...
\begin{equation}
1 - \gamma_2 ak - \gamma_1\frac{\beta_1}{\mu_1} - 2\gamma_1\left[\left(ac + \frac{\beta_0}{\mu_1}\right)\frac{\beta_2}{\mu_1}\right]^{\frac{1}{2}} > 0
\end{equation}
\end{theorem}
...
另一方面，假设 $\ddot{q}_d = 0$ 和 $\beta_2 = 0$，如果满足以下条件，则[Becker and Grimm 1988]中证明了 $e$ 的 $\mathcal{L}_2$ 稳定性：

\begin{equation}\label{eq:5.2.36}
1 - \gamma_2 ak - \gamma_1\frac{\beta_1}{\mu_1} > 0
\end{equation}
...
例 5.2--4：使用与例5.2.3相同的线性控制器，但假设 $N$ 中的二次速度项是已知的，使得 $\beta_2 = 0$；即

\begin{equation*}
\tau = \hat{M}(\ddot{q}_d - u) + V_m(q, \dot{q})\dot{q}.
\end{equation*}
...
\begin{equation}
G(s) = \frac{1}{s^2}I, \quad v = q_d
\end{equation}
则二自由度调节器描述为
\begin{align}
K_1(s) &= \frac{\omega_1^2}{s^2 + a_1\omega_1 s + \omega_1^2}I \\
K_2(s) &= \left[b_2\omega_2 s + b_1\omega_2^2 + \frac{\omega_2^3}{s}\right]I
\end{align}
其中 $s^3+b_2s^2+b_1s+1$ 是稳定多项式，$\omega_1$、$\omega_2$、$a_1$、$b_1$ 和 $b_2$ 是设计参数。注意 $K_2(s)$ 是PID控制器，由 $K_1(s)$ 给出的闭环系统是具有自然频率 $\omega_1$ 和阻尼比 $\zeta = \frac{a_1}{2}$ 的二阶系统。还注意到机器人的输入变为
\begin{equation}
\tau = K_2q + (K_2 + s^2)K_1q_d
\end{equation}
...
\begin{aligned}
\tau &= M(q)[\ddot{q}_d + C(s)e] + V_m(q, \dot{q})(\dot{q} + r) + G(q) + K_r r \\
r &= \left[sI + \frac{C(s)}{s}\right]e = F(s)^{-1}e
\end{aligned}
\end{equation}

将式(\ref{eq:5.3.1})代入式(\ref{eq:5.1.1})并假设无摩擦[即 $F(\dot{q}) = 0$]，我们得到
...
\begin{equation}
\dot{V} = r^T M(q)\dot{r} + \frac{1}{2}r^T \dot{M}(q)r
\end{equation}
...
定理 5.3--1：考虑控制律
\begin{equation}
\tau = \Lambda(s)\dot{e} + u_2
\end{equation}
其中 $\Lambda(s)$ 是SPR传递函数，由设计者选择，外部输入 $u_2$ 在 $L_2$ 范数中有界。则 $\dot{e}$ 是渐近稳定的，$e$ 是李雅普诺夫稳定的。
\end{theorem}
...
r = \Lambda(s)\dot{e}
\end{equation}
通过适当选择 $\Lambda(s)$ 和 $u_2$，可以应用被动性定理并推断 $\dot{e}$ 和 $r$ 在 $L_2$ 范数中有界，并且由于 $\Lambda(s)^{-1}$ 是SPR的（是SPR函数的逆），推断 $\dot{e}$ 是渐近稳定的，因为
\begin{equation}
\dot{e} = \Lambda(s)^{-1}r
\end{equation}
这将意味着位置误差 $e$ 是有界的但不是渐近稳定的，在时变轨迹 $[q_d^T \quad \dot{q}_d^T]^T$ 的情况下。然而，在设定点跟踪情况下（即 $\dot{q}_d = 0$），并且具有重力预补偿，可以使用LaSalle定理推断 $e$ 的渐近稳定性。只要 $\Lambda(s)$ 是SPR的并且 $u_2$ 是 $L_2$ 有界的，闭环系统的鲁棒性就得到保证，无论机器人参数的确切值如何。
\end{proof}
...
\toprule
\textbf{假设:} \\
\quad $\Lambda(s)$ 是SPR传递函数。$u_2$ 是有限能量信号。 \\[0.8em]
\midrule
\textbf{控制器:} \\[0.5em]
\quad $\displaystyle \tau = \Lambda(s)\dot{e} + u_2$ \\[1em]
\midrule
\textbf{性能:} \\
\quad 保证 $e$ 的李雅普诺夫稳定性，$\dot{e}$ 的渐近稳定性。 \\
\quad 在 $\dot{q}_d = 0$ 的情况下，也保证 $e$ 的渐近稳定性。 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{equation}
\Lambda(s) = \begin{bmatrix} \frac{s+450}{s+225} & 0 \\ 0 & \frac{s+3300}{s+10} \end{bmatrix}
\end{equation}
...
\tau_i = \begin{cases}
\tau_i^+ & \text{if } r_i(e_{1i}, \dot{q}_i) > 0 \\
\tau_i^- & \text{if } r_i(e_{1i}, \dot{q}_i) < 0
\end{cases}
\end{equation}
...
r_i(e_{1i}, \dot{q}_i) = \lambda_i e_i + \dot{q}_i, \quad \lambda_i > 0
\end{equation}
...
\begin{aligned}
r &= \Lambda e + \dot{e}, \quad \Lambda = \text{diag}[\lambda_1, \lambda_2, \ldots, \lambda_n], \quad \lambda_i > 0 \\
\dot{q}_r &= \Lambda e + \dot{q}_d \\
\text{sgn}(r_i) &= [\text{sgn}(r_1), \text{sgn}(r_2), \ldots, \text{sgn}(r_n)]^T
\end{aligned}
\end{equation}
...
\begin{equation}
\text{sgn}(r_i) = \begin{cases} +1 & \text{if } r_i > 0 \\ -1 & \text{if } r_i < 0 \end{cases}
\end{equation}

\begin{theorem}\label{thm:5.3.2}
定理 5.3--2：考虑控制器
\begin{equation}
\tau = \hat{M}\ddot{q}_r + \hat{V}_m\dot{q}_r + \hat{G} + K\text{sgn}(r),
\end{equation}
其中
\begin{equation}
...
考虑李雅普诺夫函数候选
\begin{equation}
V(r) = \frac{1}{2}r^T M(q)r
\end{equation}
然后
\begin{equation}
\dot{V}(r) = r^T M\dot{r} + \frac{1}{2}\dot{M}(q)r
\end{equation}
代入 $\dot{r}$ 并使用 $\dot{M}(q)-2V_m(q,\dot{q})$ 的斜对称性质，我们得到
\begin{equation}
\dot{V}(r) = r^T[M\ddot{q}_r + V_m\dot{q}_r + G - \tau]
\end{equation}
然后，如果我们使用由下式给出的控制器
\begin{equation}
\tau = \hat{M}\ddot{q}_r + \hat{V}_m\dot{q}_r + \hat{G} + K \text{sgn}(r)
...
我们得到
\begin{equation}
\dot{V}(r) = r^T[\tilde{M}\ddot{q}_r + \tilde{V}_m\dot{q}_r + \tilde{G}] - \sum_{i=1}^{n}k_i|r_i|
\end{equation}
其中
\begin{equation}
\tilde{M} = M - \hat{M}, \quad \tilde{V}_m = V_m - \hat{V}_m, \quad \tilde{G} = G - \hat{G}
\end{equation}
然后选择
\begin{equation}
k_i \geq |[\tilde{M}\ddot{q}_r + \tilde{V}_m\dot{q}_r + \tilde{G}]_i| + \eta_i
\end{equation}
其中 $\eta_i > 0$。因此，
\begin{equation}
\dot{V}(r) \leq -\sum_{i=1}^{n}\eta_i|r_i|
\end{equation}
...
\begin{table}[htbp]
\centering
\caption{变结构控制器（定理5.3-2）[Slotine 1985]}
\label{tab:5.3.2}
\begin{tabular}{@{}l@{}}
\toprule
\textbf{假设:} \\
\quad 无 \\[1em]
\textbf{控制器:} \\[0.5em]
\quad $\displaystyle r = \Lambda e + \dot{e}, \quad \Lambda = \text{diag}[\lambda_1, \lambda_2, \ldots, \lambda_n], \quad \lambda_i > 0$ \\[0.5em]
\quad $\displaystyle \dot{q}_r = \Lambda e + \dot{q}_d$ \\[0.5em]
\quad $\displaystyle \text{sgn}(r_i) = [\text{sgn}(r_1), \text{sgn}(r_2), \ldots, \text{sgn}(r_n)]^T$ \\[0.5em]
\quad $\displaystyle \text{sgn}(r_i) = \begin{cases} +1 & \text{if } r_i > 0 \\ -1 & \text{if } r_i < 0 \end{cases}$ \\[0.8em]
\quad $\displaystyle \tau = \hat{M}\ddot{q}_r + \hat{V}_m\dot{q}_r + \hat{G} + K\text{sgn}(r),$ \\[0.5em]
\quad $\displaystyle K = \text{diag}[k_1, k_2, \ldots, k_n], \quad k_i > 0.$ \\[1em]
\midrule
\textbf{性能:} \\
\quad 保证 $e$ 和 $\dot{e}$ 的一致最终有界性。 \\
\quad 如果 $e(0) = \dot{e}(0) = 0$，则保证一致有界性。 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{aligned}
|(M - \hat{M})_{ij}| &< \bar{M}_{ij}(q) \\
|\dot{M}_{ij}| &< \bar{m}_{ij}(q, \dot{q}) \\
|[(M - \hat{M})\ddot{q}_d]_i| &< \bar{c}_i(t) \\
|(N - \hat{N})_i| &< \bar{n}_i(q, \dot{q})
\end{aligned}
\end{equation}

注意这些界限与式(\ref{eq:5.2.9})--(\ref{eq:5.2.11})中给出的界限在精神上不同。当前界限通常更有用，因为它们依赖于惯性矩阵 $M$ 和速度相关扭矩 $N$ 的每个元素的不确定性。

\begin{theorem}\label{thm:5.3.3}
定理 5.3--3：如果输入扭矩由下式给出，则误差 $e$ 和 $\dot{e}$ 是渐近稳定的

\begin{equation}
\tau = \hat{M}\ddot{q}_r + \hat{N} + P(t)r(t) + Q(t)\text{sgn}(r),
\end{equation}
其中
\begin{equation}
\begin{aligned}
P(t) &= \text{diag}[p_1(t), p_2(t), \ldots, p_n(t)], \\
Q(t) &= \text{diag}[q_1(t), q_2(t), \ldots, q_n(t)],
\end{aligned}
\end{equation}
\begin{equation}
p_i(t) = \frac{1}{2}\sum_{j=1}^{n}\bar{m}_{ij} + k_i, \quad k_i > 0
\end{equation}
\begin{equation}
q_i(t) = \sum_{j=1}^{n}\bar{M}\Lambda_{ij}|\dot{e}_j| + \bar{n}_i + \bar{c}_i.
\end{equation}
\end{theorem}
...
然后，微分并代入 $\tau$，我们得到
\begin{equation}
\dot{V} = -r^T\left[P - \frac{\dot{M}}{2}\right]r + r^T[-Q\text{sgn}(r) + \tilde{M}\Lambda\dot{e} + \tilde{N} + \tilde{M}\ddot{q}_d],
\end{equation}
其中 $\tilde{M} = M - \hat{M}$，$\tilde{N} = N - \hat{N}$。使用(4)--(\ref{eq:5.3.8})可以证明
\begin{equation}
\dot{V} \leq -r^T K r \leq 0.
\end{equation}
因此，通过适当选择 $P$ 和 $Q$，可以保证 $\dot{V} < 0$，从而证明误差 $e$ 和 $\dot{e}$ 的渐近稳定性。
\end{proof}

\begin{table}[htbp]
\centering
\caption{变结构控制器（定理5.3-3）[Chen et al. 1990]}
\label{tab:5.3.3}
\begin{tabular}{@{}l@{}}
\toprule
\textbf{假设:} \\[0.5em]
\quad $|(M - \hat{M})_{ij}| < \bar{M}_{ij}(q)$ \\[0.5em]
\quad $|\dot{M}_{ij}| < \bar{m}_{ij}(q, \dot{q})$ \\[0.5em]
\quad $|[(M - \hat{M})\ddot{q}_d]_i| < \bar{c}_i(t)$ \\[0.5em]
\quad $|(N - \hat{N})_i| < \bar{n}_i(q, \dot{q})$ \\[1em]
\midrule
\textbf{控制器:} \\[0.5em]
\quad $\displaystyle \tau = \hat{M}\ddot{q}_r + \hat{N} + P(t)r(t) + Q(t)\text{sgn}(r)$ \\[0.8em]
\quad $\displaystyle P(t) = \text{diag}[p_1(t), p_2(t), \ldots, p_n(t)]$ \\[0.5em]
\quad $\displaystyle Q(t) = \text{diag}[q_1(t), q_2(t), \ldots, q_n(t)]$ \\[0.8em]
\quad $\displaystyle p_i(t) = \frac{1}{2}\sum_{j=1}^{n}\bar{m}_{ij} + k_i, \quad k_i > 0$ \\[0.8em]
\quad $\displaystyle q_i(t) = \sum_{j=1}^{n}\bar{M}\Lambda_{ij}|\dot{e}_j| + \bar{n}_i + \bar{c}_i$ \\[1em]
\midrule
\textbf{性能:} \\
\quad 保证 $e$ 和 $\dot{e}$ 的一致最终有界性。 \\
\quad 如果 $e(0) = \dot{e}(0) = 0$，则保证一致有界性。 \\
...
\begin{equation*}
\begin{aligned}
\hat{M} &= \begin{bmatrix} 3 & 1 \\ 1 & 1 \end{bmatrix}, \quad \bar{M} = \begin{bmatrix} 2 & 1 \\ 1 & 0 \end{bmatrix} \\
\hat{N} &= 0, \quad \bar{N} = \begin{bmatrix} 2\dot{q}_1\dot{q}_2 + \dot{q}_2^2 + 3(9.8) \\ \dot{q}_1^2 + 9.8 \end{bmatrix} \\
\bar{c} &= \begin{bmatrix} 3 \\ 1 \end{bmatrix}, \quad \Lambda = \begin{bmatrix} 3 & 0 \\ 0 & 1 \end{bmatrix}, \quad K = \begin{bmatrix} 10 \\ 10 \end{bmatrix}
\end{aligned}
\end{equation*}
...
M(q)\ddot{q} + V_m(q, \dot{q})\dot{q} + Z(q, \dot{q}) = M(q)\ddot{q} + N(q, \dot{q}) = \tau
\end{equation}
...
\|Z(q, \dot{q})\| \leq \beta_0 + \beta_1\|\dot{e}\| + \beta_2\|\dot{e}\|^2
\end{equation}

\begin{equation}\label{eq:5.3.11}
\|N(q, \dot{q})\| = \|V_m(q, \dot{q})\dot{q} + Z(q, \dot{q})\| \leq \zeta_0 + \zeta_1\|e\| + \zeta_2\|e\|^2
\end{equation}
...
\begin{equation}
\tau = \frac{2\mu_1\mu_2}{\mu_1+\mu_2}[\ddot{q}_d + K_v\dot{e} + K_p e + v_r] + \hat{N}(q,\dot{q}),
\end{equation}
其中
\begin{equation}
K_v = k_v I, \quad K_p = k_p I
\end{equation}
\begin{equation}
v_r = \begin{cases} (B^T P e)p(\|B^T P e\|)^{-1} & \text{if } \|B^T P e\| > \epsilon \\ (B^T P e)p/\epsilon & \text{if } \|B^T P e\| \leq \epsilon \end{cases}
\end{equation}
\begin{equation}
p = \frac{1}{1-a}\left[a\|\ddot{q}_d\| + k_p\|e\| + k_v\|\dot{e}\| + \frac{1}{\mu_1}\phi\right]
\end{equation}
\begin{equation}
\phi = \beta_0 + \beta_1\|e\| + \beta_2\|e\|^2
\end{equation}
\begin{equation}
a = \frac{\mu_2-\mu_1}{\mu_1+\mu_2}
\end{equation}
\end{theorem}
...
注意，在上述方程中，矩阵 $B$ 如式(\ref{eq:5.2.3})中定义，$\beta_i$ 如式(\ref{eq:5.2.10})中定义，矩阵 $P$ 是式(\ref{eq:5.3.12})中李雅普诺夫方程的对称、正定解，其中 $Q$ 是对称正定矩阵，$A_c$ 在式(\ref{eq:5.2.14})中给出。
...
Q = \begin{bmatrix} K_p & 0 \\ 0 & 2K_v - I \end{bmatrix}, \quad K_v > I
\end{equation}

导致以下 $P$：

\begin{equation}\label{eq:5.3.14}
P = \begin{bmatrix} K_p + 0.5K_v & 0.5I \\ 0.5I & I \end{bmatrix}.
\end{equation}

因此，式(\ref{eq:5.3.14})中 $P$ 的表达式可以用于 $v_r$ 的表达式。该设计总结在表5.3.4中。

\begin{table}[htbp]
\centering
\caption{饱和控制器 1（定理5.3-4）[Spong et al. 1987]}
\label{tab:5.3.4}
\begin{tabular}{@{}l@{}}
\toprule
\textbf{假设:} \\[0.5em]
\quad $\displaystyle \mu_1 I_n \leq M(q) \leq \mu_2 I_n$ \\[0.5em]
\quad $\displaystyle \|N(q, \dot{q})\| \leq \zeta_0 + \zeta_1\|e\| + \zeta_2\|e\|^2$ \\[1em]
\midrule
\textbf{控制器:} \\[0.5em]
\quad $\displaystyle \tau = \frac{2\mu_1\mu_2}{\mu_1+\mu_2}[\ddot{q}_d + K_v\dot{e} + K_p e + v_r] + \hat{N}(q, \dot{q})$ \\[1em]
\quad 其中 \\[0.5em]
\quad $\displaystyle v_r = \begin{cases} (B^T P e)p(\|B^T P e\|)^{-1} & \text{if } \|B^T P e\| > \epsilon \\ (B^T P e)p/\epsilon & \text{if } \|B^T P e\| \leq \epsilon \end{cases}$ \\[0.8em]
\quad $\displaystyle p = \frac{1}{1-a}\left[a\|\ddot{q}_d\| + k_p\|e\| + k_v\|\dot{e}\| + \frac{1}{\mu_1}\phi\right]$ \\[0.5em]
\quad $\displaystyle \phi = \beta_0 + \beta_1\|e\| + \beta_2\|e\|^2$ \\[0.5em]
\quad $\displaystyle a = \frac{\mu_2-\mu_1}{\mu_1+\mu_2}$ \\[1em]
\midrule
\textbf{性能:} \\
\quad 保证 $e$ 和 $\dot{e}$ 的一致最终有界性。 \\
\quad 如果 $e(0) = \dot{e}(0) = 0$，则保证一致有界性。 \\
...
\begin{equation}
\tau = K_v \dot{e} + v_r(p, e, \dot{e}, \epsilon),
\end{equation}
其中
\begin{equation}
v_r = \begin{cases} \left(\frac{e}{2} + \dot{e}\right)p\left(\|\frac{e}{2} + \dot{e}\|\right)^{-1} & \text{if } \|\frac{e}{2} + \dot{e}\| > \epsilon \\ \left(\frac{e}{2} + \dot{e}\right)p/\epsilon & \text{if } \|\frac{e}{2} + \dot{e}\| \leq \epsilon \end{cases}
\end{equation}
和
\begin{equation}
p = \delta_0 + \delta_1\|e\| + \delta_2\|\dot{e}\|^2
\end{equation}
其中 $\delta_i$ 是正标量。
\end{theorem}
...
和
\begin{equation}
Q = \begin{bmatrix} 0.5K_p & 0 \\ 0 & K_v \end{bmatrix}
\end{equation}
并按定理2.10.4继续进行。详情参见[Dawson et al. 1990]。
\end{proof}

注意 $p$ 不再包含伺服增益，因此可以调整 $K_p$ 和 $K_v$ 而不会影响辅助控制 $v_r$。如[Dawson et al. 1990]中所示，如果初始误差 $e(0) = 0$ 并选择 $K_v = 2K_p = k_v I$，则跟踪误差可以由以下限制，这显示了控制参数对跟踪误差的直接影响：

\begin{equation}\label{eq:5.3.15}
\|e\| \leq \left[\frac{(8k_v + 6\mu_2)\epsilon}{k_v}\right]^{\frac{1}{2}}
\end{equation}
...
\begin{table}[htbp]
\centering
\caption{饱和控制器 2（定理5.3-5）[Dawson et al. 1990]}
\label{tab:5.3.5}
\begin{tabular}{@{}l@{}}
\toprule
\textbf{假设:} \\[0.5em]
\quad $\displaystyle \mu_1 I_n \leq M(q) \leq \mu_2 I_n$ \\[1em]
\midrule
\textbf{控制器:} \\[0.5em]
\quad $\displaystyle \tau = K_v \dot{e} + v_r(p, e, \dot{e}, \epsilon)$ \\[0.5em]
\quad 其中 \\[0.5em]
\quad $\displaystyle v_r = \begin{cases} \left(\frac{e}{2} + \dot{e}\right)p\left(\|\frac{e}{2} + \dot{e}\|\right)^{-1} & \text{if } \|\frac{e}{2} + \dot{e}\| > \epsilon \\ \left(\frac{e}{2} + \dot{e}\right)p/\epsilon & \text{if } \|\frac{e}{2} + \dot{e}\| \leq \epsilon \end{cases}$ \\[0.8em]
\quad $\displaystyle p = \delta_0 + \delta_1\|e\| + \delta_2\|\dot{e}\|^2$ \\[1em]
\midrule
\textbf{性能:} \\
\quad 保证 $e$ 和 $\dot{e}$ 的一致最终有界性。 \\
\quad 如果 $e(0) = \dot{e}(0) = 0$，则保证一致有界性。 \\
...
注意最后两个控制器虽然使用连续项 $v_r$，但可以使用示例5.3.4中引入的双曲正切项进行修改。事实上，这些类型的控制器还有许多扩展。一个特别简单的扩展使用机器人未知物理参数的线性动力学，在(3.3.62)中给出，其中 $p = \|W\|\Phi_{max}$。在这种表述中，$W$ 是一个包含时变但已知项的矩阵，而 $\Phi$ 是动态公式(3.3.59)中未知但恒定项的向量，这里重复：

\begin{equation}\label{eq:5.3.16}
\tau = W(q, \dot{q}, \ddot{q})\Phi
\end{equation}
...
\begin{example}[解耦设计]
例 5.4--1：考虑图5.4.1中描述的机器人。这种机构被称为五杆连杆机构，其动力学描述在以下条件成立时：

\begin{equation*}
\frac{m_4 g_4}{m_3 g_3} = \frac{l_2}{l_1}
\end{equation*}

由下式给出

\begin{equation*}
\tau = \begin{bmatrix} I_1 + I_3 + m_1 g_1^2 + m_4 l_1^2 \left(1 + \frac{g_3 g_4}{l_1 l_2}\right) & 0 \\ 0 & I_2 + I_4 + m_3 l_2^2 + m_4 g_4^2 \left(1 + \frac{l_1 l_2}{g_3 g_4}\right) \end{bmatrix} \ddot{q}
\end{equation*}

注意惯性矩阵是解耦的且与位置无关。由下式给出的控制器

\begin{equation*}
\tau = M(\ddot{q}_d + K_v \dot{e} + K_p e)
\end{equation*}
...
\begin{equation*}
\begin{array}{llll}
m_1 = 5, & m_2 = \frac{1}{2}, & m_3 = 2, & m_4 = 1, \\
l_1 = 1, & l_2 = \frac{1}{2}, & l_3 = \frac{3}{4}, & l_4 = 1, \\
g_1 = \frac{1}{2}, & g_2 = \frac{1}{4}, & g_3 = \frac{1}{2}, & g_4 = \frac{1}{2}.
\end{array}
\end{equation*}
...
\begin{equation}
y = h(q), \label{eq:5.4.1}
\end{equation}

因此

\begin{equation}
\dot{y} = \left(\frac{\partial h}{\partial q}\right)^T \dot{q} = J\dot{q} \label{eq:5.4.2}
\end{equation}

和

\begin{equation}
\ddot{y} = J\ddot{q} + \dot{J}\dot{q}. \label{eq:5.4.3}
\end{equation}
...
M(q) = J^T(q)J(q) + \tilde{M}(q)
\end{equation}

然后使用控制器

\begin{align}
\tau &= J^T \ddot{y} + (\hat{M} - J^T J)\ddot{q} \label{eq:5.4.5} \\
\ddot{y} &= \ddot{y}_d + K_p(y_d - y) + K_v(\dot{y}_d - \dot{y}). \label{eq:5.4.6}
\end{align}
...
\begin{align}
\tau &= J^T \ddot{y} + (\hat{M} - J^T J) \ddot{q} \\
\ddot{y} &= \ddot{y}_d + K_p (y_d - y) + K_v (\dot{y}_d - \dot{y})
\end{align}

闭环系统的 $\mathcal{L}_\infty$ 稳定性如果满足以下条件则得到保证：

\begin{equation}
\|JM^{-1}(\tilde{M} - \hat{M} + J^T J)J^{-1}\| = \|J(I - M^{-1}\hat{M})J^{-1}\| < \frac{1}{2}
\end{equation}
\end{theorem}
...
\textbf{假设:} \\
\quad $y = h(q)$ \\[1em]
\quad $\dot{y} = \left(\frac{\partial h}{\partial q}\right)^T \dot{q} = J\dot{q}$ \\[1em]
\quad $M(q) = J^T(q)J(q) + \tilde{M}(q)$ \\[1em]
\quad $\|JM^{-1}(\tilde{M} - \hat{M} + J^T J)J^{-1}\| = \|J(I - M^{-1}\hat{M})J^{-1}\| < \frac{1}{2}$ \\[1em]
\midrule
\textbf{控制器:} \\
\quad $\tau = J^T \ddot{y} + (\hat{M} - J^T J)\ddot{q}$ \\[0.5em]
\quad $\ddot{y} = \ddot{y}_d + K_p(y_d - y) + K_v(\dot{y}_d - \dot{y})$ \\[1em]
\midrule
\textbf{性能:} \\
\quad 误差 $e$ 和 $\dot{e}$ 在 $\mathcal{L}_\infty$ 范数下有界。 \\
\quad 较大的增益 $K_p$ 和 $K_v$ 将减小 $e$ 和 $\dot{e}$ 的界。 \\
\bottomrule
\end{tabular}
\end{table}
...
请找到式(\ref{eq:5.2.9})--(\ref{eq:5.2.11})中定义的 $\alpha$、$\beta_i$ 和 $\mu_i$，假设 $m_{12}(q) = m_{21}(q) = 0$ 并且
