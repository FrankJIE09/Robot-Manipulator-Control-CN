...
K = B^T P = \begin{bmatrix} K_v^2/a & K_v \end{bmatrix}, \quad a > 1
\end{equation}
...
其中 $\frac{1}{2}e^T P e$ 是对应于SPR系统(\ref{eq:5.2.14})的李雅普诺夫函数。那么如果 $\Delta \geq 0$，我们有 $V > 0$。当 $\hat{M} \geq \mu_2 I$ 时满足此条件。然后微分得到

\begin{equation}
\dot{V} = -\frac{1}{2}e^T P e + eK(M^{-1}\delta - \Delta\ddot{q}_d)
\end{equation}
...
\|\delta\| \leq \beta_0 + \beta_1\|\dot{e}\| + \beta_2\|\dot{e}\|^2
\end{equation}
...
\eta = M^{-1}[\Delta M u + \Delta N] + w_1 + M^{-1}\tau_d
\end{equation}
...
\begin{equation}
\tau = M(q)\dot{r} + V_m(q, \dot{q})r + G(q) + K_r r + u_2
\end{equation}
...
\|Z(q, \dot{q}) - Z(q, \dot{q}_d)\| \leq \beta_3 + \beta_4\|e\| + \beta_5\|\dot{e}\|^2
\end{equation}
...
\begin{align}
\tau &= \hat{M}(q)(\ddot{q}_d + K_v \dot{e} + K_p e) + \hat{N}(q, \dot{q}) + v_r \\
v_r &= \frac{\beta_0 + \beta_1\|\dot{e}\| + \beta_2\|\dot{e}\|^2}{1 - a}\text{sgn}(\dot{e}) \\
\|v_r\| &\leq \frac{\beta_0 + \beta_1\|\dot{e}\| + \beta_2\|\dot{e}\|^2}{1 - a} \\
a &= \|\hat{M}^{-1}M - I\| \\
\hat{M} &\geq \mu_0 I
\end{align}
\end{theorem}
...
\begin{align}
\tau &= \hat{M}(q)\ddot{q}_r + \hat{V}_m(q, \dot{q})\dot{q}_r + \hat{G}(q) + K_v s + v_r \\
s &= \dot{e} + \Lambda e, \quad \dot{q}_r = \dot{q}_d - \Lambda e \\
v_r &= \frac{\varepsilon_0 + \varepsilon_1\|s\| + \varepsilon_2\|s\|^2}{1 - a}\text{sgn}(s)
\end{align}

其中 $\varepsilon_i$ 是正标量。
\end{theorem}
...
\begin{align}
y &= f(q) \label{eq:5.4.1} \\
\dot{y} &= J(q)\dot{q} \label{eq:5.4.2} \\
\ddot{y} &= J(q)\ddot{q} + \dot{J}(q)\dot{q} \label{eq:5.4.3}
\end{align}
...
\begin{align}
\tau &= \hat{M}(q)\nu + N(q, \dot{q}) \label{eq:5.4.5} \\
\nu &= \ddot{y}_d + K_v \dot{e}_y + K_p e_y \label{eq:5.4.6}
\end{align}
