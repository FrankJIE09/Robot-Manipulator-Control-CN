...
\begin{example}[示例4.2-1：将规定笛卡尔轨迹映射到关节空间]
...
大多数机器人控制方案需要连续的期望轨迹。为了将经过点表$q_i(t_k)$转换为各关节$i$的连续期望轨迹$q_{d_i}(t)$，我们可以使用许多选项。让我们在这里讨论多项式插值。
...
对于平滑运动，在每个时间间隔$[t_k, t_{k+1}]$上，我们要求期望位置$q_{d_i}(t)$和速度$\dot{q}_{d_i}(t)$与所制表的经过点匹配。这产生了边界条件
\begin{equation}\label{eq:4.2.2}
    \begin{cases}
        q_{d_i}(t_k) = q_i(t_k), & q_{d_i}(t_{k+1}) = q_i(t_{k+1}) \\
        \dot{q}_{d_i}(t_k) = \dot{q}_i(t_k), & \dot{q}_{d_i}(t_{k+1}) = \dot{q}_i(t_{k+1})
    \end{cases}
\end{equation}

为了匹配这些边界条件，有必要在每个时间间隔$[t_k, t_{k+1}]$上使用三次插值多项式
\begin{equation}\label{eq:4.2.3}
    q_{d_i}(t) = a_i + (t-t_k)b_i + (t-t_k)^2 c_i + (t-t_k)^3 d_i
\end{equation}
它有四个自由变量。那么
\begin{align}
    \dot{q}_{d_i}(t) &= b_i + 2(t-t_k)c_i + 3(t-t_k)^2 d_i \label{eq:4.2.4}\\
    \ddot{q}_{d_i}(t) &= 2c_i + 6(t-t_k) d_i \label{eq:4.2.5}
\end{align}
...
\begin{equation}\label{eq:4.2.6}
    \begin{bmatrix} 1 & 0 & 0 & 0 \\ 0 & 1 & 0 & 0 \\ 1 & T & T^2 & T^3 \\ 0 & 1 & 2T & 3T^2 \end{bmatrix} \begin{bmatrix} a_i \\ b_i \\ c_i \\ d_i \end{bmatrix} = \begin{bmatrix} q_i(t_k) \\ \dot{q}_i(t_k) \\ q_i(t_{k+1}) \\ \dot{q}_i(t_{k+1}) \end{bmatrix}
\end{equation}

这被求解以获得每个时间间隔$[t_k, t_{k+1}]$上所需的插值系数。实际上，我们看到
...
位置轨迹从抛物线切换到线性的时间被称为混合时间$t_b$。应为每个关节变量$i$指定位置$q_{d_i}(t)$。图4.2.4(c)中的轨迹可以写成关节$i$的形式
\begin{equation}\label{eq:4.2.8}
    q_{d_i}(t) = \begin{cases}
        a_i + (t - t_k) b_i + (t - t_k)^2 c_i, & t_k \leq t < t_k + t_b \\
        d_i + v_i t, & t_k + t_b \leq t < t_{k+1} - t_b \\
        e_i + (t - t_{k+1}) f_i + (t - t_{k+1})^2 g_i, & t_{k+1} - t_b \leq t \leq t_{k+1}
    \end{cases}
\end{equation}

系数$v_i$可以被解释为关节变量$i$允许的最大速度。设计参数是$v_i$和$t_b$。

很容易求解在每个时间间隔$[t_k, t_{k+1}]$上确保满足边界条件\eqref{eq:4.2.2}的系数。结果是
\begin{equation}\label{eq:4.2.9}
\begin{aligned}
    a_i &= q_i(t_k), & b_i &= \dot{q}_i(t_k), & c_i &= \frac{v_i - \dot{q}_i(t_k)}{2t_b}, \\
    d_i &= \frac{q_i(t_k) + q_i(t_{k+1}) - v_i t_{k+1}}{2}, \\
    e_i &= q_i(t_{k+1}), & f_i &= \dot{q}_i(t_{k+1}), \\
    g_i &= \frac{v_i t_{k+1} + q_i(t_k) - q_i(t_{k+1}) + 2t_b \bigl[\dot{q}_i(t_{k+1}) - v_i\bigr]}{2t_b^2}
\end{aligned}
\end{equation}
...
\begin{equation}\label{eq:4.3.3}
    M(q)\frac{d\dot{q}}{dt} = -N(q, \dot{q}) + \tau - \tau_d
\end{equation}
其中$\tau$是臂控制力矩，由控制器提供，$\tau_d$是干扰力矩。我们说这是一种隐式形式，因为左侧的系数矩阵意味着$\ddot{q}$不是以右侧的显式形式给出的。

给定$x(t)$，有必要为积分程序提供一个子程序来计算$\dot{x}(t)$。一种求解$\ddot{q}$的方法是求逆$M(q)$。然而，由于潜在的数值问题，这不推荐。让我们将\eqref{eq:4.3.3}表示为
\begin{equation}\label{eq:4.3.4}
    E(x)\dot{x} = f(x, u, t)
\end{equation}
请注意，在这种情况下，$u(t)$是由控制力$\tau(t)$和干扰$\tau_d(t)$组成的向量。

附录B中给出了一个简单的时间响应程序TRESP。给定一个子程序F(time, x, xp)，它使用\eqref{eq:4.3.4}根据$x(t)$和$u(t)$计算$\dot{x}$；它使用Runge-Kutta积分器来计算状态轨迹$x(t)$。为了在子程序F(t, x, xp)中求解$\dot{x}$，我们建议计算$M(q)$和$N(q, \dot{q})$，然后使用最小二乘技术求解
...
机器人手臂动力学为
\begin{align}
    M(q)\ddot{q} + V(q, \dot{q}) + F_v\dot{q} + F_d(\dot{q}) + G(q) + \tau_d &= \tau \label{eq:4.4.1}\\
    \intertext{或}
    M(q)\ddot{q} + N(q, \dot{q}) + \tau_d &= \tau, \label{eq:4.4.2}
\end{align}
其中$N(q, \dot{q}) = V(q, \dot{q}) + F_v\dot{q} + F_d(\dot{q}) + G(q)$，$q(t)$为关节变量，$\tau$为控制力矩，$\tau_d(t)$为干扰力矩。如果此动力学方程中包含电机执行器模型（第3.6节），则$\tau(t)$表示广义控制输入（例如输入电压）。
...
现在求解\eqref{eq:4.4.2}中的$\ddot{q}$并将其代入最后一个方程得到
...
定义控制输入函数和干扰函数
\begin{align}
    u &= \ddot{q}_d - M^{-1}(q)[\tau - N(q, \dot{q})] \label{eq:4.4.6}\\
    w &= M^{-1}(q)\tau_d \label{eq:4.4.7}
\end{align}
...
反馈线性化变换\eqref{eq:4.4.5}可以反转以得到
\begin{equation}\label{eq:4.4.9}
    \tau = M(q)(\ddot{q}_d - u) + N(q, \dot{q})
\end{equation}

我们称之为计算力矩控制律。这些操作的重要性如下。从\eqref{eq:4.4.2}到\eqref{eq:4.4.8}没有状态空间变换。因此，如果我们选择一个稳定\eqref{eq:4.4.8}的控制$u(t)$，使得$e(t)$趋于零，那么由$\tau(t)$\eqref{eq:4.4.9}给出的非线性控制输入将使得满足式\eqref{eq:4.4.2}的机器人手臂实现轨迹跟随。事实上，将\eqref{eq:4.4.9}代入\eqref{eq:4.4.2}得到
\begin{align}
    M\ddot{q} + N + \tau_d &= M(\ddot{q}_d - u) + N \label{eq:4.4.10}\\
    \ddot{e} &= u + M^{-1}\tau_d = u + w \label{eq:4.4.11}
\end{align}
这正是(4.4.8)。
...
$H(s)$可以被选择用于良好的闭环行为。根据\eqref{eq:4.4.11}，闭环误差系统然后具有传递函数
...
\begin{equation}\label{eq:4.4.20}
    \Delta_c(s) = \prod_{i=1}^{n}\left(s^2 + k_{vi} s + k_{pi}\right)
\end{equation}
只要所有的$k_{vi}$和$k_{pi}$都为正，误差系统就是渐近稳定的。因此，只要干扰$w(t)$有界，误差$e(t)$就是有界的。与此相关，检查\eqref{eq:4.4.6}并回忆表3.3.1，$M^{-1}$是有上界的。因此，$w(t)$的有界性等同于$\tau_d(t)$的有界性。
...
\begin{equation}\label{eq:4.4.21}
    k_{p_i} = \omega_{ni}^2, \quad k_{v_i} = 2\zeta_i\omega_{ni}
\end{equation}
其中$\zeta_i,\omega_{ni}$分别为关节误差$i$的期望阻尼比与自然频率。使末端附近的响应比靠近基座处更快可能是有用的，因为必须移动的质量更重。

机器人表现出超调是不希望的，因为如果例如期望轨迹在工件的表面终止，这可能导致冲击。因此，PD增益通常选择为各关节临界阻尼（$\zeta_i = 1$）。在这种情况下，
\begin{equation}\label{eq:4.4.22}
    k_{v_i} = 2\sqrt{k_{p_i}}, \quad k_{p_i} = \frac{k_{v_i}^2}{4}
\end{equation}
当$\zeta_i=1$时，由式\eqref{eq:4.4.21}亦得$k_{v_i}=2\omega_{ni}$与$k_{p_i}=\omega_{ni}^2$，与式\eqref{eq:4.4.22}一致。
...
如下从误差有界性考虑提供了PD增益选择的一些更多感觉。由式\eqref{eq:4.4.15}，闭环误差向量在拉普拉斯域满足
\begin{equation}\label{eq:4.4.25}
    e(s) = (s^2 I + K_v s + K_p)^{-1} w(s)
\end{equation}
若$K_v$和$K_p$为对角阵，则第$i$个误差分量可写为
\begin{equation}\label{eq:4.4.26}
\begin{aligned}
    e_i(s) &= \frac{1}{s^2 + k_{v_i} s + k_{p_i}} w(s) \equiv H(s) w(s), \\
    \dot{e}_i(s) &= \frac{s}{s^2 + k_{v_i} s + k_{p_i}} w(s) \equiv s H(s) w(s)
\end{aligned}
\end{equation}
其中在该分量上$H(s)$表示相应的标量传递函数（依赖于$i$，下文同）。又
\begin{equation*}
    w = M^{-1} \tau_d .
\end{equation*}
假定$w$与$M^{-1}$有界（表3.3.1），则对所论机器人臂存在常数$\bar{m},\bar{d}$使得
\begin{equation}\label{eq:4.4.27}
    \|w\| \leq \|M^{-1}\| \, \|\tau_d\| \leq \bar{m} \bar{d}
\end{equation}
因此
\begin{equation}\label{eq:4.4.28}
    \|e_i(t)\| \leq \|H(s)\| \, \|w\| \leq \|H(s)\| \, \bar{m} \bar{d}
\end{equation}
\begin{equation}\label{eq:4.4.29}
    \|\dot{e}_i(t)\| \leq \|s H(s)\| \, \|w\| \leq \|s H(s)\| \, \bar{m} \bar{d}
\end{equation}
选取$L_2$范数时，算子增益$\|H(s)\|_2$可取为$H(s)$的Bode幅频曲线的峰值。对临界阻尼系统有
\begin{equation}\label{eq:4.4.30}
    \sup_{\omega} \|H(\mathrm{j}\omega)\|_2 = \frac{1}{k_{p_i}}
\end{equation}
故
\begin{equation}\label{eq:4.4.31}
    \|e_i(t)\|_2 \leq \frac{\bar{m} \bar{d}}{k_{p_i}}
\end{equation}
此外（见习题），
\begin{equation}\label{eq:4.4.32}
    \sup_{\omega} \|\mathrm{j}\omega H(\mathrm{j}\omega)\|_2 = \frac{1}{k_{v_i}}
\end{equation}
从而
\begin{equation}\label{eq:4.4.33}
    \|\dot{e}_i(t)\|_2 \leq \frac{\bar{m} \bar{d}}{k_{v_i}}
\end{equation}

更一般地，若闭环极点由阻尼比$\zeta_i$与自然频率$\omega_{ni}$参数化，则
\begin{equation*}
    |H_i(\mathrm{j}\omega)| = \frac{1}{\sqrt{(\omega^2 - \omega_{ni}^2)^2 + (2\zeta_i\omega_{ni}\omega)^2}}
\end{equation*}
据此有$\|H(s)\|_2 = \max_i \bigl[1/(2\zeta_i\omega_{ni}^2)\bigr]$，$\|sH(s)\|_2 = \max_i \bigl[1/(2\zeta_i\omega_{ni})\bigr]$，并对应$\|e_i\|_2 \leq \bar{m}\bar{d}/\omega_{ni}^2$、$\|\dot{e}_i\|_2 \leq \bar{m}\bar{d}/\omega_{ni}$（当$\zeta_i=1$时，$k_{p_i}=\omega_{ni}^2$、$k_{v_i}=2\omega_{ni}$，与式\eqref{eq:4.4.31}–\eqref{eq:4.4.33}一致）。

因此，在临界阻尼的情况下，位置误差界随$k_{p_i}$增大而减小，速度误差界随$k_{v_i}$增大而减小。

\begin{example}[示例4.4-1：PD计算力矩控制的仿真]
\textbf{a. 计算力矩控制律}
...
\begin{example}[示例4.4-2：PID计算力矩控制的仿真]
...
为了提高性能，可以使用PID外环控制。定义积分状态$\varepsilon$满足
\begin{equation}\label{eq:4.4.34}
    \dot{\varepsilon} = e
\end{equation}
并取辅助信号
\begin{equation}\label{eq:4.4.35}
    u = -K_v\dot{e} - K_pe - K_i\varepsilon
\end{equation}
由此得到机械臂控制输入
\begin{equation}\label{eq:4.4.36}
    \tau = M(q)\left(\ddot{q}_d + K_v\dot{e} + K_pe + K_i\varepsilon\right) + N(q, \dot{q})
\end{equation}

此控制器如图4.4.6所示。或以状态空间形式，我们有
\begin{equation}\label{eq:4.4.37}
    \frac{\dd}{\dd t}\begin{bmatrix} \varepsilon \\ e \\ \dot{e} \end{bmatrix} = \begin{bmatrix} 0 & I & 0 \\ 0 & 0 & I \\ 0 & 0 & 0 \end{bmatrix}\begin{bmatrix} \varepsilon \\ e \\ \dot{e} \end{bmatrix} + \begin{bmatrix} 0 \\ 0 \\ I \end{bmatrix}u + \begin{bmatrix} 0 \\ 0 \\ I \end{bmatrix}w
\end{equation}
将式\eqref{eq:4.4.35}代入$\ddot{e}=u+w$，得闭环误差动力学
\begin{equation}\label{eq:4.4.38}
    \ddot{e} + K_v\dot{e} + K_pe + K_i\varepsilon = w
\end{equation}

闭环特征多项式为
\begin{equation}\label{eq:4.4.39}
    \Delta_c(s) = |s^3 I + K_v s^2 + K_p s + K_i|
\end{equation}
...
一类计算力矩型（computed-torque-like）控制器可通过对计算力矩控制律作如下修改而得到：
\begin{equation}\label{eq:4.4.43}
    \tau_c = \hat{M} (\ddot{q}_d - u) + \hat{N}
\end{equation}
其中$\hat{M}$和$\hat{N}$是$M$和$N$的近似值，可能在实际实现中使用以节省计算量。然后闭环系统变为
\begin{equation}\label{eq:4.4.44}
    \ddot{e} = u - \Delta u + \delta
\end{equation}
或等价地写成
\begin{equation}\label{eq:4.4.45}
    (I - \Delta)\ddot{e} + K_v\dot{e} + K_pe + \delta = w
\end{equation}
其中
\begin{align}
    \Delta &= I - \hat{M}^{-1}M \label{eq:4.4.46}\\
    \delta &= \hat{M}^{-1}(N - \hat{N}) \label{eq:4.4.47}\\
    w &= \hat{M}^{-1}\tau_d \label{eq:4.4.48}
\end{align}

如果使用精确计算力矩控制使得$\Delta = 0, \delta = 0$，这会简化为误差系统\eqref{eq:4.4.11}。否则，误差系统由期望加速度和非线性项不匹配$\delta(t)$驱动。因此，跟踪误差永远不会精确为零。此外，辅助控制$u(t)$乘以$(I - \Delta)$，这可能导致一个非常困难的控制问题。

一类常用的特例是PD结合重力补偿控制器（详见下一小节），其控制律为
\begin{equation}\label{eq:4.4.49}
    \tau_c = K_v\dot{e} + K_pe + G(q).
\end{equation}

使用外环PD反馈，使得$u(t) = -K_v\dot{e} - K_pe$，得到误差系统
\begin{equation}\label{eq:4.4.50}
    (I - \Delta)\ddot{e} + K_v\dot{e} + K_pe + d = w
\end{equation}
其中$d(t) = \delta + \Delta\ddot{q}_d$。即使$K_v$和$K_p$被选择为左侧具有良好的稳定性，这种系统的行为也不是显而易见的。存在两种问题：首先是干扰项$d(t)$，其次是误差二阶导数的函数$\Delta\ddot{e}$。

\subsection{PD-Plus-Gravity控制器}

计算力矩族中一个有用的控制器是PD-plus-gravity控制器，当$\hat{M} = I, \hat{N} = G(q) - \ddot{q}_d$时产生，其中$G(q)$是操作臂动力学的重力项。然后，为$u(t)$选择PD反馈即可得到式(\ref{eq:4.4.49})（与式(\ref{eq:4.4.15})所用外环记号一致）。

此控制律在[Arimoto and Miyazaki 1984]、[Schilling 1990]中进行了处理。它比精确计算力矩控制器实现起来简单得多。

当臂静止时，动力学\eqref{eq:4.4.1}中唯一的非零项是重力$G(q)$、干扰$\tau_d$和控制力矩$\tau$。PD-gravity控制器$\tau_c$包含$G(q)$，因此我们应该期望对设定点跟踪有良好的性能，即当给定恒定的$q_d$使得$\ddot{q}_d = 0$时。下一个结果将这一点形式化。它依赖于Lyapunov证明（第1章），这种证明在本书中将始终有用，特别是利用表3.3.1中的斜对称性质。因此，理解此证明中的步骤非常重要。

\begin{theorem}
假设在式\eqref{eq:4.4.2}所给出的臂动力学中使用PD-gravity控制且$\tau_d = 0, \ddot{q}_d = 0$。则稳态跟踪误差$e = q_d - q$为零。
\end{theorem}

\begin{proof}
\textbf{1. 闭环系统}

忽略摩擦时可取式\eqref{eq:4.4.2}并把$N$理解为$V(q,\dot{q})+G(q)$（等价于式\eqref{eq:4.4.1}中取$F_v=F_d=0$）。

当$\ddot{q}_d = 0$时，将控制律\eqref{eq:4.4.49}代入式\eqref{eq:4.4.2}（取$\tau=\tau_c$、$\tau_d=0$）得到闭环动力学
\begin{equation*}
    M\ddot{q} + V_m\dot{q} - K_v\dot{e} - K_pe = 0.
\end{equation*}
...
所展示的稳定性表明$\dot{q}$和$q$是有界的，因此闭环动力学与$M^{-1}(q)$的有界性（见表3.3.1）揭示$\ddot{q}$是有界的。因此，$\ddot{V}$是有界的，因此$\dot{V}$是一致连续的。这保证了根据Barbalat引理，$\dot{V}$趋于零。

现在很明显$\dot{q}$趋于零。还需要表明跟踪误差$e(t)$消失。注意当$\dot{q} = 0$时，有$\dot{e}=-\dot{q}=0$，于是闭环动力学化为
\begin{equation*}
    \ddot{q} = M^{-1}(q)K_pe.
\end{equation*}

在集合$\dot{V}\equiv 0$内的轨迹上有$\dot{V}=-\dot{q}^TK_v\dot{q}\equiv 0$，因而$\dot{q}\equiv 0$，随之$\ddot{q}\equiv 0$；代入闭环动力学即得$K_pe=0$，故$e=0$（$K_p$正定）。因此包含于$\{x \mid \dot{V}=0\}$的最大不变集仅为原点$x=0$，进而$e(t)$与$\dot{q}(t)$均趋于零，证明结束。
\end{proof}

\begin{example}[示例4.4-3：PD-Gravity控制器的仿真]
在示例4.4.1中，我们在双连杆平面操作臂上仿真了精确的计算力矩控制律。在这里，让我们仿真PD-gravity控制器。我们将采用相同的臂参数和期望轨迹。

假设每个连杆的PD增益相同（标量$k_p,k_v$），双连杆臂的PD-gravity控制律可写成分量形式（重力项与示例4.4-1动力学方程中的重力向量一致，此处记$c_1=\cos\theta_1$，$c_{12}=\cos(\theta_1+\theta_2)$）：
\begin{align*}
    \tau_{c1} &= k_v\dot{e}_1 + k_pe_1 + (m_1+m_2)ga_1 c_1 + m_2ga_2 c_{12}, \\
    \tau_{c2} &= k_v\dot{e}_2 + k_pe_2 + m_2ga_2 c_{12}.
\end{align*}
等价地亦可写成$\tau_c = K_v\dot{e} + K_pe + G(q)$；当各关节增益相同时$K_v=k_vI$，$K_p=k_pI$，而$G(q)$即上式中的重力部分。
...
然后有结果
\begin{equation}\label{eq:4.4.51}
    \tau_c = -u.
\end{equation}
...
\begin{equation}\label{eq:4.4.52}
    \left(J_m + r_i^2 m_{ii}\right)\ddot{\theta}_i + \left(B_m + \frac{k_b k_m}{R}\right)\dot{\theta}_i = \frac{k_m}{R}v_i - r_i d_i, \quad i = 1,\ldots,n.
\end{equation}
...
\begin{equation}\label{eq:4.4.53}
    J\ddot{\theta}_i + B\dot{\theta}_i = k v_i - r d_i
\end{equation}
...
\textbf{PD控制}。选择PD控制律
\begin{equation}\label{eq:4.4.54}
    u = k_v\dot{e} + k_pe
\end{equation}
...
\begin{equation}\label{eq:4.4.55}
    \theta = \frac{k_p}{\Delta(s)}\theta_d - \frac{r}{\Delta(s)}d
\end{equation}
具有闭环特征多项式
\begin{equation}\label{eq:4.4.56}
    \Delta_c(s) = Js^2 + (B + k_v)s + k_p
\end{equation}
...
\begin{equation}\label{eq:4.4.57}
...
\begin{theorem}
如果将PD控制律\eqref{eq:4.4.54}应用于每个关节且$e(0) = 0, \dot{e}(0) = 0$，则位置和速度跟踪误差在球内有界，其半径随着$k_v \to \infty$大致以$1/\sqrt{k_v}$减小。
\end{theorem}
...
在[Paul 1981]中讨论了修改PD控制律以获得更好性能的几种方法。这些包括重力补偿[这恰好产生控制器\eqref{eq:4.4.49}]和加速度前馈，这相当于使用
\begin{equation}\label{eq:4.4.58}
    u = k_v\dot{e} + k_pe + J\ddot{\theta}_{di}.
\end{equation}
对于每个关节。还提到了关节耦合控制，这相当于在$u(t)$中重新添加$M(q)$和$N(q, \dot{q})$中描述关节之间相互作用的一些被忽略的项。因此，这种修正涉及在近似计算力矩控制\eqref{eq:4.4.43}中使用更好的$\hat{M}$和$\hat{N}$估计。

\textbf{PID控制}。已经看到PD独立关节控制通常适合跟踪控制。然而，在稳态时存在由于重力引起的残余误差\eqref{eq:4.4.57}。这可以使用PID独立关节控制律消除：
\begin{align}
    \dot{\varepsilon} &= e \notag \\
    u &= k_v\dot{e} + k_pe + k_i\varepsilon \label{eq:4.4.59}
\end{align}
对于每个关节。

现在设定点跟踪（$\dot{\theta}_d = 0$）的传递函数为
\begin{equation}\label{eq:4.4.60}
    \theta = \frac{k_ps + k_i}{\Delta_1(s)}\theta_d - \frac{rs}{\Delta_1(s)}d
\end{equation}
具有闭环特征多项式
\begin{equation}\label{eq:4.4.61}
    \Delta_1(s) = s^3 J + s^2(B + k_v) + sk_p + k_i
\end{equation}
现在终值定理显示，对于设定点控制，稳态误差为零。Routh检验表明，对于稳定性，需要
\begin{equation}\label{eq:4.4.62}
    k_i < \frac{(B + k_v)k_p}{J}
\end{equation}

在控制律中使用积分项时，重要的是要注意由于执行器饱和限制引起的积分器饱和的可能性，这在本节前面的"PID外环设计"中进行了讨论。

\begin{example}[示例4.4-4：经典关节控制和力矩饱和限制]
...
\end{equation*}
其中$\varepsilon$是积分状态。这个简单的定律很容易通过向图4.4.2中的子程序arm(x, xp)添加两个状态$x(5)$和$x(6)$来实现，以考虑积分器（参见示例4.4.2）。
...
\hline
\textbf{机器人动力学} & \begin{minipage}[t]{\linewidth}\raggedright\setlength{\lineskip}{5pt}
$\displaystyle M(q)\ddot{q}+V(q,\dot{q})+F_v\dot{q}+F_d(\dot{q})+G(q)+\tau_d=\tau$\par
$\displaystyle M(q)\ddot{q}+N(q,\dot{q})+\tau_d=\tau$，\quad $\displaystyle N(q,\dot{q})\equiv V(q,\dot{q})+F_v\dot{q}+F_d(\dot{q})+G(q)$\par
\end{minipage} \\
\hline
\textbf{跟踪误差} & $\displaystyle e(t)=q_d(t)-q(t)$ \\
\hline
\textbf{PD计算力矩} & $\displaystyle \tau=M(q)(\ddot{q}_d+K_v\dot{e}+K_pe)+N(q,\dot{q})$ \\
\hline
\textbf{PID计算力矩} & $\displaystyle \dot{\varepsilon}=e$，\quad $\displaystyle \tau=M(q)(\ddot{q}_d+K_v\dot{e}+K_pe+K_i\varepsilon)+N(q,\dot{q})$ \\
\hline
\textbf{近似计算力矩} & $\displaystyle \tau_c=\hat{M}(\ddot{q}_d-u)+\hat{N}$ \\
\hline
\textbf{误差系统} & \begin{minipage}[t]{\linewidth}\raggedright\setlength{\lineskip}{5pt}
$\displaystyle \ddot{e}=(I-\Delta)u+d$\par
$\displaystyle \Delta=I-M^{-1}\hat{M}$，\quad $\displaystyle \delta=M^{-1}(N-\hat{N})$，\par
$\displaystyle d(t)=M^{-1}\tau_d+\Delta\ddot{q}_d(t)+\delta(t)$\par
\end{minipage} \\
\hline
\textbf{PD--重力} & $\displaystyle \tau_c=K_v\dot{e}+K_pe+G(q)$ \\
\hline
\textbf{经典PD} & $\displaystyle \tau_c=K_v\dot{e}+K_pe$ \\
\hline
\textbf{经典PID} & $\displaystyle \dot{\varepsilon}=e$，\quad $\displaystyle \tau_c=K_v\dot{e}+K_pe+K_i\varepsilon$ \\
\hline
\textbf{数字控制}（见第4.5节） & $\displaystyle \tau_k=M(q_k)\bigl(\ddot{q}_k^{\mathrm{d}}+K_v\dot{e}_k+K_pe_k\bigr)+N(q_k,\dot{q}_k)$ \\
\hline
\end{tabular}
\end{center}
...
一种数字机器人控制方案如图4.5.1所示：在对$q(t)$与$\dot{q}(t)$采样后定义
\begin{equation}\label{eq:4.5.1}
\begin{aligned}
    q_k &\equiv q(kT), \\
    \dot{q}_k &\equiv \dot{q}(kT),
\end{aligned}
\end{equation}
其中$T$为采样周期。机器人应用中的采样周期通常在约$1\sim20\ \mathrm{ms}$之间。采用\textbf{零阶保持}(ZOH)由离散样本$\tau_k$重构驱动执行器所需的连续时间控制力矩$\tau(t)$。本节中用\textbf{上标$\mathrm{d}$}标记期望轨迹以简化记号。

该数字控制律相当于在表4.4.1的近似计算力矩控制器中取
\begin{equation}\label{eq:4.5.2}
\begin{aligned}
    \hat{M} &= M\bigl(q(kT)\bigr)=M(q_k), \\
    \hat{N} &= N\bigl(q(kT),\dot{q}(kT)\bigr)=N(q_k,\dot{q}_k),
\end{aligned}
\end{equation}
并对\textbf{外环}采用离散控制信号。于是，在\textbf{PD外环}下机械臂控制输入为
\begin{equation}\label{eq:4.5.3}
    \tau_k = M(q_k)\bigl(\ddot{q}_k^{\mathrm{d}} + K_v\dot{e}_k + K_pe_k\bigr) + N(q_k,\dot{q}_k),
\end{equation}
其中跟踪误差为$e(t)=q_d(t)-q(t)$（因而$e_k=e(kT)$等）。该结构有多种变体：例如可采用\textbf{多采样率}，使内环（非线性）更新慢于外环线性反馈；鉴于计算力矩控制的鲁棒性，这在实践中往往效果很好。

\subsection{采样时的性能保证}

通常在连续时间中设计控制器并给出稳定性与误差有界性证明；实现时则选取``足够小''的采样周期，稳定性依赖仿真验证。下述结果表明这种做法并非毫无依据。定义\textbf{误差状态向量}
\begin{equation*}
    \underline{e}=\begin{bmatrix} e \\ \dot{e} \end{bmatrix}.
\end{equation*}

\begin{theorem}
假设将\textbf{PD数字控制律}\eqref{eq:4.5.3}施加于机械臂动力学。则对每个$L>0$，存在$T_M$，使得对所有$T<T_M$，任一误差轨迹若在某一时刻$t_0$满足$\|e(t_0)\|\le L$，则对所有$t\ge t_0$及任意$r>0$都有$\|e(t)\|\le L+r$。
\end{theorem}

\begin{proof}
采用上述数字控制律可得误差系统（由表4.4.1，采样情形下$\Delta$、$\delta$取下述形式）
\begin{equation*}
    \ddot{e} = u - \Delta u + d,
\end{equation*}
其中
\begin{equation*}
    d = M^{-1}(q)\tau_d + \Delta \ddot{q}_d + \delta,
\end{equation*}
且
\begin{equation*}
\begin{aligned}
    \Delta &= M^{-1}(q)\bigl[M(q)-M(q_k)\bigr], \\
    \delta &= M^{-1}(q)\bigl[N(q,\dot{q})-N(q_k,\dot{q}_k)\bigr].
\end{aligned}
\end{equation*}

定义$\underline{e}=\mathrm{col}(e,\dot{e})$，可将误差动力学写成状态空间形式
\begin{equation*}
    \underline{\dot{e}} = \begin{bmatrix} 0 & I \\ 0 & 0 \end{bmatrix}\underline{e} + \begin{bmatrix} 0 \\ I \end{bmatrix}(u-\Delta u+d) \equiv A\underline{e}+B(u-\Delta u+d).
\end{equation*}

施加\textbf{外环数字控制}
\begin{equation*}
    u=-K\underline{e}_k=-K\underline{e}+K(\underline{e}-\underline{e}_k),
\end{equation*}
得到闭环
\begin{equation*}
    \underline{\dot{e}}=(A-BK)\underline{e}+BK(\underline{e}-\underline{e}_k)+B(\Delta K\underline{e}_k+d).
\end{equation*}

选取$K$使$(A-BK)$稳定；因而存在正定矩阵$P,Q$满足Lyapunov方程
\begin{equation*}
    (A-BK)^{\mathsf{T}}P+P(A-BK)=-Q.
\end{equation*}

取Lyapunov函数
\begin{equation*}
    V(\underline{e})=\frac{1}{2}\underline{e}^{\mathsf{T}}P\underline{e},
\end{equation*}
则由闭环与上式可得
\begin{equation*}
    \dot{V}(\underline{e})=-\frac{1}{2}\underline{e}^{\mathsf{T}}Q\underline{e}+\underline{e}^{\mathsf{T}}PB\bigl[K(\underline{e}-\underline{e}_k)+d+\Delta K\underline{e}_k\bigr].
\end{equation*}

为简化起见设$\tau_d=0$，则在每个采样时刻有
\begin{equation*}
    \dot{V}(\underline{e}_k)=-\frac{1}{2}\underline{e}_k^{\mathsf{T}}Q\underline{e}_k.
\end{equation*}
它在采样瞬时\textbf{负定}。[若$\tau_d\neq 0$但有界，则$\dot{V}$在某球外亦可为负，讨论可相应修改。]

证明的剩余部分借助[LaSalle and Lefschetz 1961]的定理，需验证：（1）当$\|\underline{e}\|>L$时$\dot{V}(\underline{e})\le 0$；（2）当$\|\underline{e}(t_1)\|\le L$且$\|\underline{e}(t_2)\|>L+r$（$t_2\ge t_1\ge 0$）时有$V(\underline{e}(t_1))<V(\underline{e}(t_2))$。关键在于$\dot{V}(\underline{e}_k)$在每个采样点的性质以及$\dot{V}(\underline{e})$的连续性。
\end{proof}

上述结果表明：存在依赖于具体机械臂的最大采样周期$T_M$，使得只要$T<T_M$即可获得良好跟踪；更细致的分析还可说明最大期望加速度越大则误差越大，等价地需要更小的采样周期。

我们现在讨论\textbf{内环非线性}与\textbf{外环线性}离散化中的若干问题。

\subsection{内环非线性的离散化}

离散化非线性动力学并不存在方便的精确形式。给定非线性状态方程
...
也存在在采样瞬时守恒能量与动量的``精确''离散化方法[Neuman and Tourassis 1985]，参见[Elliott 1990]；遗憾的是即便对简单机械臂，所得离散动力学也极为复杂，难以据此推导有保证的数字控制律。本节将\textbf{内环非线性}模块直接视为由近似\eqref{eq:4.5.2}给出。

\subsection{由位置测量估计关节速度}

本章示例在连续时间仿真中常假定关节位置与速度均可精确测量；事实上关节速度多由编码器位置测量\textbf{估计}而非直接测量。若用Euler公式直接差分
\begin{equation}\label{eq:4.5.6}
    \dot{q}_k=\frac{q_k-q_{k-1}}{T}
\end{equation}
则近乎注定失败，因为该高通运算会放大编码器噪声。

记关节速度估计为$v_k$，可由\textbf{滤波差分}由$q_k$计算：
\begin{equation}\label{eq:4.5.7}
    v_k=\nu v_{k-1}+\frac{q_k-q_{k-1}}{T},
\end{equation}
其中$\nu$为设计参数。$\nu$较小时对应$z$平面靠近原点处的快速极点，可对传感器噪声提供一定高通衰减。示例4.5-1将演示该速度估计滤波器的使用。

还可针对给定噪声统计用$\alpha$-$\beta$跟踪器优化设计[Lewis 1986a]、[Lewis 1986b]、[Lowe and Lewis 1991]（Kalman滤波的一种特化）。应注意：速度估计不仅用于\textbf{外环}计算反馈，也用于在\eqref{eq:4.5.3}中计算\textbf{内环}非线性项$N(q_k,\dot{q}_k)$。

\subsection{外环PD/PID控制的离散化}

我们已经看到，一类有用的计算力矩\textbf{外环}反馈是PID控制器。给定连续时间PID外环，可按如下思路设计数字外环[Lewis 1992]。
...
\begin{equation}\label{eq:4.5.11}
    K(z) = K_v\frac{z - 1}{T} + K_p + \frac{K_i}{\frac{z - 1}{T}} = K_v\frac{z - 1}{T} + K_p + \frac{K_iT}{z - 1}
\end{equation}
...
\begin{example}[示例4.5-1：数字机器人计算力矩控制器的仿真]
...
\begin{example}[示例4.5-2：带抗饱和补偿的数字PI控制器]
采样周期为$T$秒的一般数字PI控制器由下式给出
\begin{equation*}
    u_k = k_p e_k + k_i T \sum_{i=0}^{k} e_i
\end{equation*}
...
乘以$(1-z^{-1})$写成
\begin{equation*}
    (1-z^{-1})U(z) = k_p(1-z^{-1})E(z) + k_i T E(z)
\end{equation*}
...
\begin{align*}
    u_k &= u_{k-1} + k_p(e_k - e_{k-1}) + k_i T e_k + (1-\alpha)(u_{k-1} - u_{k-1}^{sat}) \\
    u_k^{sat} &= \mathrm{sat}(u_k)
\end{align*}

如果$\alpha=1$，我们获得特殊情况，称为位置形式，没有抗饱和补偿。如果$\alpha=0$，我们获得死拍抗饱和补偿
\begin{equation*}
    u_k = u_{k-1} + k_p(e_k - e_{k-1}) + k_i T e_k + (u_{k-1} - u_{k-1}^{sat})
\end{equation*}
...
\begin{equation*}
    \tau_k = k_p e_k + k_i T \sum_{i=0}^{k} e_i
\end{equation*}
...
可控性在第1章中讨论过。可观测性大致意味着所有系统模式在PI中都有独立的影响，因此如果$J$有界，所有模式都独立地随$t$趋于零。验证这些性质很容易。若可控性矩阵
\begin{equation}\label{eq:4.6.7}
    U = \begin{bmatrix} B & AB & A^2B & \cdots & A^{n-1}B \end{bmatrix}
\end{equation}
具有满秩$n$，则$(A,B)$可控。系统$(A,C)$可观测，当且仅当可观测性矩阵
\begin{equation}\label{eq:4.6.8}
    V = \begin{bmatrix} C \\ CA \\ CA^2 \\ \vdots \\ CA^{n-1} \end{bmatrix}
\end{equation}
具有满秩$n$。MATLAB等为这些检验提供标准函数。对本节的LQ设计，常取$C=\sqrt{Q}$，与本定理中“$\sqrt{Q},A$可观测”的假设一致。因此，可适当选择状态加权矩阵$Q$以满足可观测性要求。

该定理使这种现代设计方法非常强大。无论输入和状态的数量如何，只要假设成立，就可以找到稳定系统的反馈增益。

\subsection{线性二次计算力矩设计}

现在我们将这些结果应用于机器人操作臂动力学的控制。臂动力学为
...
\begin{equation}\label{eq:4.6.10}
    \tau = M(\ddot{q}_d - u) + N
\end{equation}
其中为书写简洁记$M=M(q)$，$N=N(q,\dot{q})$。将\eqref{eq:4.6.10}代入\eqref{eq:4.6.9}，在4.4节与$\tau_d=0$时相同的理想情形下（即$\ddot{e}=u$），误差系统可写为
\begin{equation}\label{eq:4.6.11}
    \frac{\dd}{\dd t}\begin{bmatrix} e \\ \dot{e} \end{bmatrix} = \begin{bmatrix} 0 & I \\ 0 & 0 \end{bmatrix}\begin{bmatrix} e \\ \dot{e} \end{bmatrix} + \begin{bmatrix} 0 \\ I \end{bmatrix}u
\end{equation}
或标准状态空间形式
...
\end{equation}
状态取为
\begin{equation}\label{eq:4.6.13}
    x = \begin{bmatrix} e \\ \dot{e} \end{bmatrix}
\end{equation}
其中$A=\begin{bmatrix} 0 & I \\ 0 & 0 \end{bmatrix}$，$B=\begin{bmatrix} 0 \\ I \end{bmatrix}$。若存在扰动$\tau_d$，则如4.4节有$\ddot{e}=u+M^{-1}\tau_d$。

现在，选择外环PD反馈
\begin{equation}\label{eq:4.6.14}
    u = -Kx = -\begin{bmatrix} K_p & K_v \end{bmatrix}x = -K_pe - K_v\dot{e}
\end{equation}

为了找到稳定增益$K$，在PI中取设计矩阵
\begin{equation*}
    Q = \mathrm{diag}\{Q_p, Q_v\}, \quad Q_p, Q_v \in \mathbb{R}^{n \times n}
\end{equation*}
使得位置和速度误差被独立加权。然后，由于代表$n$个解耦牛顿定律（即双积分器）系统的简单$A$和$B$矩阵形式，Riccati方程的解很容易找到（参见习题）。将该解用于\eqref{eq:4.6.5}可得最优稳定增益公式
\begin{equation}\label{eq:4.6.15}
...
\begin{equation}\label{eq:4.6.16}
    \tau_c = \hat{M} (\ddot{q}_d - u) + \hat{N}
\end{equation}
...
\begin{equation}\label{eq:4.6.17}
    \tau = M (\ddot{q}_d + K_v\dot{e} + K_pe) + N
\end{equation}
其中$M=M(q)$，$N=N(q,\dot{q})$。尽管使用这种方法没有最小化$\tau(t)$中的能量，但我们可以使用一些范数不等式来写入
\begin{equation}\label{eq:4.6.18}
    \|\tau\| \leq \|M(q)\| \cdot \|\ddot{q}_d\| + \|M(q)\| \cdot \|u(t)\| + \|N(q, \dot{q})\|
\end{equation}
...
这种方法与3.4节的一般反馈线性化思路一致：从关节空间动力学出发，在工作空间中对末端位姿进行跟踪。末端笛卡尔位姿$y(t)$如何选取与测量所涉及的问题，可参见附录A。

关节空间动力学为
\begin{equation}\label{eq:4.7.1}
    M(q)\ddot{q} + N(q, \dot{q}) + \tau_d = \tau
\end{equation}
其中$\tau_d$为扰动力矩。感兴趣的输出取为笛卡尔误差
\begin{equation}\label{eq:4.7.2}
    e_y(t) = y_d(t) - y(t)
\end{equation}
其中$y_d(t)$为期望笛卡尔轨迹，$y(t)$为末端执行器笛卡尔位姿。注意$y(t)$不必是6维向量（位置+姿态）表示：例如可用$4\times 4$齐次变换（臂$T$矩阵）描述位姿，此时$y(t)$与$y_d(t)$均可理解为$4\times 4$矩阵，\eqref{eq:4.7.2}在矩阵意义上理解。

期望轨迹可用齐次变换表示为
\begin{equation}\label{eq:4.7.3}
    y_d(t) = T_d(t) = \begin{bmatrix} n_d(t) & o_d(t) & a_d(t) & p_d(t) \\ 0 & 0 & 0 & 1 \end{bmatrix}
\end{equation}
其中$n_d,o_d,a_d,p_d$为基坐标系下的期望姿态轴与位置。实际位姿$y(t)$亦可取欧拉角6向量、四元数7向量，或在$\mathbb{R}^6$中编码的工具位形向量等形式（应与\eqref{eq:4.7.2}中采用的参数化一致）。

笛卡尔误差常写成6维向量
\begin{equation}\label{eq:4.7.4}
    e_y = \begin{bmatrix} e_p \\ e_o \end{bmatrix}
\end{equation}
其中$e_p(t)$为位置误差，$e_o(t)$为姿态误差。

运动学关系为
\begin{equation}\label{eq:4.7.5}
    y = h(q)
\end{equation}
其中$h$表示从关节变量$q(t)$到任务空间坐标$y(t)$的映射。相应的雅可比为$J = \partial h/\partial q$，且
\begin{equation}\label{eq:4.7.6}
    \dot{y} = J\dot{q}
\end{equation}

相对于$e_y(t)$的笛卡尔计算力矩控制由下式给出
\begin{equation}\label{eq:4.7.7}
    \tau = M J^{-1} (\ddot{y}_d - \dot{J}\dot{q} - u) + N
\end{equation}
其中$M=M(q)$，$N=N(q,\dot{q})$。代入\eqref{eq:4.7.1}得到误差系统
...
\begin{equation}\label{eq:4.7.9}
    w = JM^{-1}\tau_d
\end{equation}
我们称\eqref{eq:4.7.7}为笛卡尔计算力矩控制律。

外环控制$u(t)$可使用已经提到的关节空间计算力矩外环设计（参见表4.4.1）。对于PD控制，将$u=-K_v\dot{e}_y-K_pe_y$代入\eqref{eq:4.7.7}，得完整控制律
\begin{equation}\label{eq:4.7.10}
    \tau = M J^{-1}\left(\ddot{y}_d - \dot{J}\dot{q} + K_v\dot{e}_y + K_pe_y\right) + N
\end{equation}
其中$M=M(q)$，$N=N(q,\dot{q})$。

笛卡尔计算力矩控制的一个缺点是需要在每个采样周期计算逆Jacobian。为减轻计算负担，可采用近似控制律
\begin{equation}\label{eq:4.7.11}
    \tau_c = \hat{C}\left(\ddot{y}_d - \dot{J}\dot{q} - u\right) + \hat{N}
\end{equation}
其中$\hat{C}$与$\hat{N}$分别为$MJ^{-1}$与$N$的近似。通过取$\hat{C}=I$、$\hat{N}=-\ddot{y}_d+\dot{J}\dot{q}+G(q)$，可得笛卡尔PD–gravity控制器
...
可由关节变量与臂齐次变换矩阵得到实际笛卡尔位姿
\begin{equation}\label{eq:4.7.14}
    y(t) = T(t) = \begin{bmatrix} n(t) & o(t) & a(t) & p(t) \\ 0 & 0 & 0 & 1 \end{bmatrix}
\end{equation}
并可类似地写出期望位姿$y_d(t)=T_d(t)$（见式\eqref{eq:4.7.3}）。定义实际与期望速度
\begin{equation}\label{eq:4.7.15}
    \dot{y} = \begin{bmatrix} v \\ \omega \end{bmatrix}, \quad \dot{y}_d = \begin{bmatrix} v_d \\ \omega_d \end{bmatrix}
\end{equation}
其中$v(t)$、$v_d(t)$为线速度，$\omega(t)$、$\omega_d(t)$为角速度。笛卡尔速度误差取为
\begin{equation}\label{eq:4.7.16}
    \dot{e}_y = \dot{y}_d - \dot{y}
\end{equation}
由测量关节变量可确定$\dot{y}(t)$（例如$\dot{y}=J\dot{q}$）。线位置误差为
\begin{equation}\label{eq:4.7.17}
    e_p = p_d - p
\end{equation}

分别表示式\eqref{eq:4.7.14}与式\eqref{eq:4.7.3}中齐次变换的旋转部分为$R(t)$与$R_d(t)$。姿态误差可表示为绕Euler轴$k(t)$转角$\phi(t)$、把$R(t)$带到$R_d(t)$的旋转。定义3向量
\begin{equation}\label{eq:4.7.18}
    e_o(t) = k(t)\sin\phi(t), \quad -\frac{\pi}{2} \leq \phi(t) \leq \frac{\pi}{2}
\end{equation}
并常假设$e_o(t)$很小。由$T(t)$与$T_d(t)$可用
\begin{equation}\label{eq:4.7.19}
    e_o = \frac{1}{2}(n \times n_d + o \times o_d + a \times a_d)
\end{equation}
计算$e_o$。整体笛卡尔误差写为$e_y = [e_p^{\mathsf{T}} \, e_o^{\mathsf{T}}]^{\mathsf{T}}$。

需注意：采用\eqref{eq:4.7.18}时，一般而言$\dot{e}_y$并不是$e_y$对时间的导数；尽管如此，控制律\eqref{eq:4.7.10}在实践中仍常能取得良好效果。[Wu and Paul 1982]还给出采用叉乘矩阵及采样周期$T$等别的误差定义。
...
\item \textbf{带近似CT控制的误差动力学}。考虑示例3.2.1中的双连杆极坐标臂，具有形式为$F(\dot{q})=F_v\dot{q}+K_d\text{sgn}(\dot{q})$的摩擦。对以下情况找到误差动力学\eqref{eq:4.4.46}：
\begin{enumerate}[label=(\alph*)]
\item 摩擦未包含在CT控制律中。
