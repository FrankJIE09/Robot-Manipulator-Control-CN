...
\left\{
\begin{aligned}
        a_{i0} &= q_{ik}, & a_{i1} &= 0, & a_{i2} &= \frac{v_i}{2t_b} \\
        b_{i0} &= q_{ik} - \frac{v_i t_b}{2}, & b_{i1} &= v_i \\
        c_{i0} &= q_{i,k+1} - \frac{v_i t_f^2}{2t_b}, & c_{i1} &= \frac{v_i t_f}{t_b}, & c_{i2} &= -\frac{v_i}{2t_b}
\end{aligned}
\right.
\end{equation}
...
定义控制输入函数和干扰函数
\begin{align}
    u &= M^{-1}(q)[\tau - N(q, \dot{q})] - \ddot{q}_d \\
    w &= M^{-1}(q)\tau_d
\end{align}
...
\begin{equation}
    \|w\|_2 \leq \|M^{-1}\|_2\|\tau_d\|_2 \leq \bar{m}\|\tau_d\|_2 \leq d
\end{equation}
...
闭环特征多项式为
\begin{equation}
    \Delta_c(s) = |s^3I + s^2K_v + sK_p + K_i|
\end{equation}
...
\begin{equation}\label{eq:4.4.48}
...
\begin{equation}
    \left(J_{mi} + r_i^2 m_{ii}\right)\ddot{\theta}_i + \left(B_{mi} + \frac{k_{bmi}k_{mi}}{R_i}\right)\dot{\theta}_i + r_i d_i = \frac{k_{mi}}{R_i}v_i
\end{equation}
...
\begin{equation}\label{eq:4.4.55}
    \frac{\Theta_i}{\Theta_{di}} = \frac{k_vs + k_p}{Js^2 + (B + k_v)s + k_p}
\end{equation}
具有闭环特征多项式
...
\begin{equation}\label{eq:4.4.60}
    \frac{\Theta_i}{\Theta_{di}} = \frac{k_vs^2 + k_ps + k_i}{Js^3 + (B + k_v)s^2 + k_ps + k_i}
\end{equation}
具有闭环特征多项式
...
\begin{equation}
    K_p = \sqrt{Q_p R^{-1}}, \quad K_v = \sqrt{2\sqrt{Q_p R^{-1}}}
\end{equation}
...
\begin{equation}
    e_o = \frac{1}{2}(n_d \times n + o_d \times o + a_d \times a)
\end{equation}
