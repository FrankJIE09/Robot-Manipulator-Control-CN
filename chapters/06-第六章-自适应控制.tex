...
\begin{equation}\label{eq:6.2.error-def}
e = q_d - q
\end{equation}
...
\begin{equation}\label{eq:6.2.error-system}
...
\begin{equation}\label{eq:6.2.W-matrix}
...
\begin{equation}\label{eq:6.2.param-error}
...
跟踪变量 & $r(s) = H^{-1}(s)e(s)$ \\ \hline
传递函数 & $H^{-1}(s) = \left[sI_n + \frac{1}{s}K(s)\right]$ \\ \hline
...
\begin{align}
\dot{\hat{m}} &= [p_1(\dot{f} * \dot{q}) + p_2(f * \dot{q})]\tilde{\tau}_f \\
\dot{\hat{b}} &= [p_2(\dot{f} * \dot{q}) + p_3(f * \dot{q})]\tilde{\tau}_f
\end{align}
...
\dot{\hat{\varphi}} = \Gamma Y^T(\cdot)r - P W_f^T(\cdot)\tilde{\tau}_f
\end{equation}
...
\textbf{控制器} & \\ \hline
扭矩控制 & $\tau = Y(\cdot)\hat{\varphi} + K_v r$ \\ \hline
自适应更新规则 & $\dot{\hat{\varphi}} = \Gamma Y^T(\cdot)r - P W_f^T(\cdot)\tilde{\tau}_f$ \\ \hline
辅助信号 & $r = \dot{e} + \Lambda e$ \\ \hline
...
\begin{align}
\dot{\hat{m}} &= \gamma_1 Y_{11} r - (p_1 W_{f11} + p_2 W_{f12})\tilde{\tau}_f \\
\dot{\hat{b}} &= \gamma_2 Y_{12} r - (p_2 W_{f11} + p_3 W_{f12})\tilde{\tau}_f
\end{align}
