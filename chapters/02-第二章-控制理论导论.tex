...
\[
\ddot{q} = M^{-1}(q)[\tau - V(q,\dot{q})\dot{q} - G(q)]
\]
或
\[
\dot{x} = \begin{bmatrix} \dot{q} \\ M^{-1}(q)[\tau - V(q,\dot{q})\dot{q} - G(q)] \end{bmatrix}
\]
其中
\[
f(x) = \begin{bmatrix} x_2 \\ M^{-1}(x_1)[-V(x_1,x_2)x_2 - G(x_1)] \end{bmatrix}, \quad g(x) = \begin{bmatrix} 0 \\ M^{-1}(x_1) \end{bmatrix}
\]
\end{example}
