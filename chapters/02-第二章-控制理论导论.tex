...
\begin{aligned}
&\frac{d^n y(t)}{dt^n} + a_{n-1}\frac{d^{n-1}y(t)}{dt^{n-1}} + \cdots + a_1\frac{dy(t)}{dt} + a_0y(t) \\
&\quad = b_n\frac{d^n u(t)}{dt^n} + b_{n-1}\frac{d^{n-1}u(t)}{dt^{n-1}} + \cdots + b_1\frac{du(t)}{dt} + b_0u(t)
\end{aligned}
\end{equation}
其中 $a_i, b_i, i=0,\ldots,n$ 是标量常数，$y(t)$ 是标量输出，$u(t)$ 是标量输入。此外，对某时刻 $t_0$ 给定初始条件：
\begin{equation}
y(t_0),\, \frac{dy}{dt}(t_0),\, \ldots,\, \frac{d^{n-1}y}{dt^{n-1}}(t_0),\, u(t_0),\, \frac{du}{dt}(t_0),\, \ldots,\, \frac{d^{n-1}u}{dt^{n-1}}(t_0)
\end{equation}
注意，输入 $u(t)$ 的导数次数最多与输出 $y(t)$ 相同。否则，系统被称为非动态的。

\subsubsection{状态空间实现}

系统的状态被定义为一组充分的变量，当在时间 $t_0$ 指定这些变量以及输入 $u(t), t \geq t_0$ 时，足以完全确定系统在 $t \geq t_0$ 时的行为[Kailath 1980]。状态向量然后包含确定系统中任何信号的未来行为所需的所有必要变量。根据定义，这样的状态向量 $x(t)$ 不是唯一的，这一特性稍后将加以利用。事实上，如果 $x$ 是状态向量，那么任何 $\bar{x}(t)=Tx(t)$ 也是状态向量，其中 $T$ 是任何 $n \times n$ 可逆矩阵。对于在\eqref{eq:2.2.1}中描述的连续时间系统，可以选择状态变量 $x_i(t) = d^{i-1}y(t)/dt^{i-1}$，其中 $x_1=y, x_2=\dot{y}, \ldots, x_n=y^{(n-1)}$。状态空间描述如下：
...
\vdots & \vdots \\
\dot{x}_{n-1}(t) &= x_n(t) \\
\dot{x}_n(t) &= -a_0x_1(t) - a_1x_2(t) - \cdots - a_{n-1}x_n(t) + u(t)
\end{aligned}
\end{equation}
其中 $\dot{x}_i = dx_i/dt, i=1,2,\ldots,n$。输入-输出方程然后简化为：
\begin{equation}\label{eq:2.2.3}
y(t) = b_0 x_1(t) + b_1 x_2(t) + \cdots + b_{n-1} x_n(t) + b_n u(t)
\end{equation}
...
{\small\itshape
\hspace*{2em}\textbf{[校对者注：符号含义说明]}

\hspace*{2em}状态空间方程中各符号的物理含义：
\begin{itemize}
    \item $\boldsymbol{x}$ \textbf{(状态向量)}：系统内部的状态变量，包含描述系统行为所需的全部信息。如机器人关节的角度 $x_1$、角速度 $x_2$、角加速度 $x_3$ 等。$x_1$ 是位置，$x_2=\dot{x}_1$ 是速度，$x_3=\dot{x}_2$ 是加速度。
    
    \item $\boldsymbol{u}$ \textbf{(输入/控制量)}：外部施加的控制作用，如电机转矩、电压、力等。这是我们可以主动调节的量。
    
    \item $\boldsymbol{y}$ \textbf{(输出/观测量)}：通过传感器实际测量得到的量，如编码器读数（角度）、陀螺仪读数（角速度）。$y$ 可以是 $x$ 的任意组合，取决于传感器的类型和安装位置。
\end{itemize}

\hspace*{2em}以机器人单关节为例：若只用编码器测角度，则 $y=x_1$；若同时用陀螺仪测角速度，则输出方程包含 $x_1$ 和 $x_2$ 两项。
}
...
c &= \begin{bmatrix} b_0 & b_1 & \cdots & b_{n-1} \end{bmatrix}, \quad d = b_n
\end{aligned}
\end{equation}
...
\begin{example}[双平台系统]
考虑图2.2.2所示的MIMO机械系统，它代表了一个用于将实验与外部扰动隔离的双平台系统。系统有两个输入：导致地面移动的 $u_2$ 和导致平台 $m_1$ 移动的 $u_1$。系统还有2个输出，即平台 $m_1$ 的运动 $y_1$ 和平台 $m_2$ 的运动 $y_2$。实验将在平台 $m_1$ 的顶部进行，因此，人们希望最小化 $y_1$ 的大小。系统包含弹簧 $k_1$、$k_2$ 和阻尼器 $c_1$、$c_2$，使用牛顿第二定律获得描述该系统的微分方程：
\[
m_1 \ddot{y}_1 + c_1(\dot{y}_1 - \dot{y}_2) + k_1(y_1 - y_2) + u_1 = 0
\]
\[
m_2 \ddot{y}_2 + c_1(\dot{y}_2 - \dot{y}_1) + c_2\dot{y}_2 + k_1(y_2 - y_1) + k_2(y_2 - u_2) = 0
\]
...
\]
\[
\dot{x}_2 = -\frac{k_1}{m_1}(x_1 - x_3) - \frac{c_1}{m_1}(x_2 - x_4) - \frac{u_1}{m_1}
\]
\[
\dot{x}_3 = x_4
\]
\[
\dot{x}_4 = -\frac{k_1}{m_2}(x_3 - x_1) - \frac{k_2}{m_2}(x_3 - u_2) - \frac{c_1}{m_2}(x_4 - x_2) - \frac{c_2}{m_2}x_4
\]
\end{example}

\begin{quote}
\textit{注：读者可以通过}\href{https://frankjie09.github.io/Robot-Manipulator-Control-CN/figures/Two_Platform_System.html}{\textbf{交互式可视化工具}}\textit{观察双平台弹簧-质量-阻尼系统的动态响应，调节参数（质量、弹簧刚度、阻尼系数）并观察外部激励对两个平台的影响。}
\end{quote}
...
\end{aligned}
\end{equation}
其中 $\mathbf{X}(s), \mathbf{U}(s)$ 和 $\mathbf{Y}(s)$ 分别是 $\mathbf{x}(t), \mathbf{u}(t)$ 和 $\mathbf{y}(t)$ 的拉普拉斯变换。初始状态向量为 $\mathbf{x}(0)$。通过在\eqref{eq:2.2.7}中的两个方程之间消去 $\mathbf{X}(s)$，我们找到以下关系：
\begin{equation}\label{eq:2.2.8}
\mathbf{Y}(s) = [\mathbf{C}(s\mathbf{I}-\mathbf{A})^{-1}\mathbf{B} + \mathbf{D}]\mathbf{U}(s) + \mathbf{C}(s\mathbf{I}-\mathbf{A})^{-1}\mathbf{x}(0)
\end{equation}
如前所述，当 $\mathbf{x}(0)=0$ 时，传递函数作为输入 $\mathbf{U}(s)$ 和输出 $\mathbf{Y}(s)$ 之间的关系获得，即：
...
P(s) = \mathbf{C}(s\mathbf{I}-\mathbf{A})^{-1}\mathbf{B} + \mathbf{D}
\end{equation}
使得（见图2.2.1）：
\begin{equation}\label{eq:2.2.11}
\mathbf{Y}(s) = P(s)\mathbf{U}(s)
\end{equation}
...
&\quad = b_n u(k+n) + b_{n-1} u(k+n-1) + \cdots + b_1 u(k+1) + b_0 u(k),
\end{aligned}
\end{equation}
...
以与连续时间情况类似的方式，定义状态变量 $x_i(k) = y(k+i-1)$，其中 $i=1,2,\ldots,n$。状态空间描述如下：
...
y(k) = b_0 x_1(k) + b_1 x_2(k) + \cdots + b_{n-1}x_n(k) + b_n u(k)
\end{equation}
...
\end{aligned}
\end{equation}
其中 $\mathbf{A}$ 是 $n \times n$，$\mathbf{B}$ 是 $n \times m$，$\mathbf{C}$ 是 $p \times n$，$\mathbf{D}$ 是 $p \times m$。
...
以与连续时间情况类似的方式，由\eqref{eq:2.2.17}给出的线性、时不变、离散时间系统可以在Z变换域中由其传递函数 $P(z)$ 描述，从输入 $\mathbf{U}(z)$ 到输出 $\mathbf{Y}(z)$，使得：
\[
\mathbf{Y}(z) = P(z)\mathbf{U}(z)
\]
其中
\[
P(z) = \mathbf{C}(z\mathbf{I}-\mathbf{A})^{-1}\mathbf{B} + \mathbf{D}
\]
...
示例2.2.4的传递函数由下式给出：
\[
\frac{Y(z)}{U(z)} = \frac{T^2}{2}\,\frac{z+1}{(z-1)^2}
\]
\end{example}
...
\frac{d^n y(t)}{dt^n} = h\bigl[y(t), y^{(1)}(t), \ldots, y^{(n-1)}(t), u(t), u^{(1)}(t), \ldots, u^{(n)}(t)\bigr]
\end{equation}
其中 $y(t)$ 是输出，$u(t)$ 是所考虑系统的输入。与线性情况一样，定义状态分量 $x_1=y,\ x_2=\dot{y},\ \ldots,\ x_n=y^{(n-1)}$，则状态方程可写为
...
\vdots & \vdots \\
\dot{x}_{n-1}(t) &= x_n(t) \\
\dot{x}_n(t) &= h\bigl[x_1(t), x_2(t), \ldots, x_n(t), u(t), u^{(1)}(t), \ldots, u^{(n)}(t)\bigr]
\end{aligned}
\end{equation}
输出方程然后简化为：
...
其中
\[
U(t) = [u(t) \quad u^{(1)}(t) \quad \cdots \quad u^{(n-1)}(t)]^T
\]
和
...
\[
\begin{bmatrix} \dot{x}_1 \\ \dot{x}_2 \end{bmatrix} = \begin{bmatrix} x_2 \\ -\sin x_1 - kx_2 \end{bmatrix}
\]
$y(t)$ 的时间历程如图2.3.1所示。
...
刚性机器人由以下方程描述：
\[
M(q)\ddot{q} + V(q, \dot{q})\dot{q} + G(q) = \tau
\]
...
\]
输入向量为 $u=\tau$，假设输出向量为 $y=q$。由于刚性机器人的一些特殊性质（见第3章），矩阵 $M(q)$ 已知是可逆的，因此：
\[
\begin{bmatrix} \dot{q} \\ \ddot{q} \end{bmatrix} = \begin{bmatrix} \dot{q} \\ -M^{-1}(q)\bigl(V(q,\dot{q})\dot{q} + G(q)\bigr) \end{bmatrix} + \begin{bmatrix} 0 \\ M^{-1}(q) \end{bmatrix} \tau
\]
或等价地，以 $y=q=[I \ \ 0]x$ 的输出，
\[
\dot{x} = F(x) + G(x)u = f(x,u), \qquad y = \begin{bmatrix} I & 0 \end{bmatrix} x
\]
其中
\[
F(x) = \begin{bmatrix} x_2 \\ -M^{-1}(x_1)\bigl(V(x_1,x_2)x_2 + G(x_1)\bigr) \end{bmatrix}, \quad G(x) = \begin{bmatrix} 0 \\ M^{-1}(x_1) \end{bmatrix}
\]
（与上文同一状态划分下，也可记 $f(x)=F(x)$、$g(x)=G(x)$，使得 $\dot{x}=f(x)+g(x)u$。）
\end{example}
...
y(k+n) = h\bigl[y(k+n-1), \ldots, y(k+1), y(k), u(k), u(k-1), \ldots, u(k+n)\bigr]
\end{equation}
其中 $y(\cdot)$ 和 $u$ 如前所定义。状态变量的简单选择将导致：
...
\vdots & \vdots \\
x_{n-1}(k+1) &= x_n(k) \\
x_n(k+1) &= h\bigl[x_n(k), \ldots, x_2(k), x_1(k), u(k), u(k-1), \ldots, u(k+n)\bigr]
\end{aligned}
\end{equation}
或更紧凑地：
...
\begin{example}[逻辑斯蒂映射]
考虑标量系统
\[
y(k+1) = \lambda\, y(k)\bigl[1 - y(k)\bigr]
\]
这导致状态空间表示：
\[
x(k+1) = \lambda\, x(k)\bigl[1 - x(k)\bigr], \quad y(k) = x(k)
\]
\end{example}
...
在本节中，我们集中于由\eqref{eq:2.3.4}描述的系统，附加要求是 $u(t)$ 被指定为状态 $x(t)$ 的函数，即：
...
\begin{example}[函数范数示例]

1. 函数 $f(t)=e^{-t}$ 属于 $L_1$。事实上，$||e^{-t}||_1=1$。函数 $f(t)=\frac{1}{t+1}$ 属于 $L_2$。正弦函数 $f(t)=2\sin t$ 属于 $L_\infty$，因为其幅值被2限制，且 $||2\sin t||_\infty=2$。

2. 假设向量函数 $x(t)$ 具有连续且实值的分量，即：
\[
x(t) = [x_1(t), x_2(t), \ldots, x_n(t)]^T
\]
那么
\[
||x||_\infty = \max_i \sup_{t \geq 0} |x_i(t)|
\]
\end{example}

在某些情况下，我们希望处理在有限时间内有界但可能随着时间趋于无穷而变得无界的信号。这导致我们定义扩展的 $L_p$ 空间。因此考虑函数
\begin{equation}\label{eq:2.5.1}
f_T(t) = \begin{cases} f(t), & t \leq T \\ 0, & t > T \end{cases}
\end{equation}
那么，扩展的 $L_p$ 空间定义为
\[
L_{pe} = \{f | f_T \in L_p \text{ 对所有 } T < \infty\}
\]
我们还定义 $L_{pe}$ 上的范数为
\begin{equation}\label{eq:2.5.2}
|||f|||_{T_p} = ||f_T||_p
\end{equation}
感兴趣的读者可以参考 [Boyd and Barratt]、[Desoer and Vidyasagar 1975] 了解更多细节。

\begin{example}[扩展 $L_p$ 空间]
函数 $f(t)=t$ 属于 $L_{pe}$（对于任何 $p \in [1, \infty]$），但不属于 $L_p$。
\end{example}
...
\begin{example}[内积示例]
$\mathbb{R}^n$ 中的通常点积是内积。具体来说，对于 $x, y \in \mathbb{R}^n$：
\[
\langle x, y \rangle = x^T y = \sum_{i=1}^{n} x_i y_i
\]

这也是有限维空间中最常用的内积。该内积诱导的范数是欧几里得范数：
\[
...
\begin{example}[刚性机器人PD控制器的稳定性]
考虑刚性机器人动力学
\[
M(q)\ddot{q} + V(q, \dot{q})\dot{q} + G(q) = \tau
\]
其中 $M(q)$ 是惯性矩阵，$V(q, \dot{q})$ 包含科里奥利力和离心力项，$G(q)$ 是重力向量。

考虑PD控制律
\[
\tau = K_p e + K_v \dot{e} + G(q)
\]
其中 $e = q_d - q$ 是跟踪误差，$K_p$ 和 $K_v$ 是正定增益矩阵。
...
对时间求导并代入闭环动力学，利用斜对称性质 $\dot{M} - 2V(q, \dot{q})$，可得
\[
\dot{V} = -\dot{q}^T K_v \dot{q} \leq 0
\]

这表明系统是稳定的，且跟踪误差将收敛到零。
\end{example}
...
\]
其中，\textbf{正定}是指 $V(t,0)=0$ 且对所有 $x \neq 0$ 有 $V(t,x)>0$；\textbf{半负定}是指对所有 $x$ 有 $\dot{V}(t,x) \leq 0$；\textbf{负定}是指 $\dot{V}(t,0)=0$ 且对所有 $x \neq 0$ 有 $\dot{V}(t,x)<0$；\textbf{递减}是指存在$K$类函数$\alpha$使得 $V(t,x) \leq \alpha(||x||)$；\textbf{径向无界}是指当 $||x|| \rightarrow \infty$ 时 $V(t,x) \rightarrow \infty$。$\dot{V}$ 表示 $V$ 沿系统轨迹的全导数：$\dot{V} = \frac{\partial V}{\partial t} + \frac{\partial V}{\partial x}f(t,x)$。
...
\begin{quote}
\textit{注：读者可以通过}\href{https://frankjie09.github.io/Robot-Manipulator-Control-CN/figures/Lyapunov_Stability_Demo.html}{\textbf{交互式可视化工具}}\textit{直观理解李雅普诺夫稳定性定理中的各种稳定性类型（稳定、渐近稳定、指数稳定、全局渐近稳定、不稳定），观察不同条件下的状态轨迹演变。}
\end{quote}
...
尽管李雅普诺夫稳定性理论并不直接给出系统的瞬态行为指示，但它实际上可以用于估计线性系统的收敛速率。为了看清这一点，考虑以下不等式，它们是 Rayleigh-Ritz 定理 2.5.2 的结果：
\begin{equation}\label{eq:2.7.2}
\lambda_{\min}(P) x^T x \leq x^T P x \leq \lambda_{\max}(P) x^T x
\end{equation}
那么，注意
\begin{equation}\label{eq:2.7.3}
\dot{V} \leq -\gamma x^T x
\end{equation}
现在，我们可以通过分离变量并积分来证明
\begin{equation}\label{eq:2.7.4}
x^T \lambda_{\min}(P) x \leq V(t) \leq V(0) e^{-\alpha t}
\end{equation}
或者
\begin{equation}\label{eq:2.7.5}
||x(t)|| \leq \sqrt{V(0)/\lambda_{\min}(P)} e^{-\alpha t/2}
\end{equation}
因此，$x(t)$ 以快于 $\alpha/2$ 的速率趋近于原点。事实上，可以证明当 $Q=I$ 时获得最快的收敛速率估计。
...
\textbf{线性控制设计问题}：考虑如下LTI系统：
\begin{equation}\label{eq:2.11.1}
\dot{x} = Ax + Bu
\end{equation}
设计一个对输出 $y(t)$ 进行操作而不进行微分并生成输入 $u(t)$ 的反馈控制器，使得系统从任何初始状态 $x(0)$ 在有限时间内达到指定的期望最终状态 $x_d$。这个问题有2个部分：
\begin{enumerate}
...
\begin{theorem}[可控性测试]
LTI系统完全可控的必要和充分条件是 $n \times nm$ 可控性矩阵：
...
LTI系统完全可观测的必要和充分条件是 $np \times n$ 可观测性矩阵：
...
假设我们有2个通过状态空间变换关联的传递函数$H(s)$的实现：
\[
\begin{aligned}
\dot{x} &= Ax + bu \\
y &= cx + du
...
y &= cT\bar{x} + du = \bar{c}\bar{x} + \bar{d}u
\end{aligned}
\]
那么，变换后的可控性矩阵$\bar{\mathcal{C}}$为：
\begin{equation}\label{eq:2.11.7}
\begin{aligned}
\bar{\mathcal{C}} &= [T^{-1}b \quad T^{-1}ATT^{-1}b \quad \cdots \quad T^{-1}A^{n-1}b] \\
&= T^{-1}\mathcal{C}
\end{aligned}
\end{equation}
因此，$\text{rank}\mathcal{C} = \text{rank}\bar{\mathcal{C}}$，可控性在状态空间变换下保持不变。

接下来要解决的问题是找到系统\eqref{eq:2.11.1}的条件，使得静态输出反馈控制器将使闭环系统SPR。我们只考虑SISO情况，因为这是本书中唯一遇到的情况。
\begin{equation}\label{eq:2.11.8}
\begin{aligned}
\dot{x} &= (A - \gamma BKC)x + BK\tau \\
y &= Cx
\end{aligned}
\end{equation}
或在频域中
\begin{equation}\label{eq:2.11.9}
Y(s) = [I + \gamma P(s)K]^{-1}P(s)KR(s) = \frac{P(s)K}{1+\gamma P(s)K}R(s)
\end{equation}
我们提出一个简单的频域结果来展示存在$K$和$\gamma$使得闭环系统SPR。该结果首次出现在[Gu 1988]中。
